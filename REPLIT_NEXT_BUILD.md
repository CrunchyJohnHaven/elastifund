...
- Add `/live/` now.
- Allow one hidden `/build/` route for internal contributor cohorts; keep it out of the primary nav.
...
The build contract is artifact-driven, not prose-driven.

- Runtime launch/service posture comes from runtime truth artifacts.
- Fund-level realized ARR stays blocked until claim posture clears.
- Public proof combines BTC5 sleeve status, capped conservative headline fields, and JJ-N launch-readiness fields.
- Refresh all listed artifacts before each publish and treat stale values as blocked/default posture.

### Artifact-derived currentness checklist

- `reports/public_runtime_snapshot.json` and `reports/runtime_truth_latest.json` agree on launch posture and service health.
- `improvement_velocity.json` is used for all trading proof + forecast labels and must keep raw/label separation.
- `reports/nontrading_public_report.json` and `reports/nontrading_launch_summary.json` are the JJ-N source of truth.
- `public_updated_at`, `source`, and confidence fields must be visible whenever a metric is surfaced.
- Any stale or contradictory artifacts must keep `release_posture=shadow_ready_after_upgrade` and avoid headline refresh.
...
| `/build/` | hidden cohort onboarding packet | `/develop/` sources plus setup FAQ, report-back contract, and dispatch link |
...
- Must stay out of the primary nav.
- Must answer fork vs clone, local vs shared hub, secrets required, Docker optionality, and what to report back after the first 20 minutes.
- Must keep the first-pass story paper-mode only.
- Must link back to `/develop/` and the canonical onboarding dispatch doc.
- Must explicitly forbid live credential, wallet, treasury, and personal-VPS dependency for first-pass onboarding.
...
1. Open `/`, `/live/`, `/leaderboards/trading/`, `/leaderboards/worker/`, `/elastic/`, `/develop/`, and `/build/`.
