...
- the checked-in runtime/public snapshot now captures the current broad system posture: `565` cycles, `5` local trade-db trades, `4` local open positions, `0%` realized system ARR, `jj-live.service` `stopped`, and launch posture still `blocked`
- the dedicated BTC5 sleeve is now explicitly the only intended live lane: the latest checked-in truth comes from `remote_sqlite_probe`, not a local fallback DB, and shows `38` `live_filled` rows, positive cumulative live-filled outcomes, latest live fill at `2026-03-09T17:30:00Z`, and a still-positive last-12-fill slice
- BTC5 edge framing is now concrete enough for the site: best filled price bucket is `<0.49`, `0.50` remains the weak bucket, and the best replay guardrails are still `max_abs_delta=0.00015`, `UP max=0.49`, `DOWN max=0.51`
...
- remote wallet truth in the same checked-in refresh is materially ahead of the local trade DB: `19` open positions, `25` closed positions, and positive realized outcomes
