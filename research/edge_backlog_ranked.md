...
**Version:** 3.7.1
**Date:** 2026-03-12
**Flywheel Cycle:** 3 (Maker-First Queue Reset)
...
| **Parked — Structural Alpha (A-6/B-1, Cycle 3)** | **2** |
| Tested & Rejected | 10 |
| Pre-Rejected (v3, <10% P(Works) + impractical) | 8 |
| Re-Evaluating (maker-only) | 1 |
| Program blueprints (system-level, non-edge) | 1 |
...
**Structural-alpha park note (2026-03-11, Cycle 3, Instance 6):** Both A-6 and B-1 are **PARKED THIS CYCLE** (`decision=parked_this_cycle` in `reports/research/structural_alpha/structural_alpha_decision.json`). A-6 `kill_decision=kill` (upper_confidence_bound=0.0583 < 0.50 over 62 cycles, `fill_probability_gate=insufficient_data`, `settlement_path_gate=untested`). B-1 at 0 deterministic template pairs in 1000-market audit. Neither lane consumes live-priority slots this cycle. Reopen A-6 only with new fill-probability and settlement evidence. Reopen B-1 only after 50-pair gold set validated at >=85%.

**Refactor-program activation note (2026-03-11):** Imported HFT architecture research is now integrated as an active system program with a phased blueprint at `docs/ops/hft_refactor_blueprint.md` and a machine-readable task contract at `reports/hft_refactor/latest.json`. This program is a gating layer above individual edge promotion and does not change the `131` edge count.

**Fast-market queue reset note (2026-03-12, Instance 6):** The next fast-market research cycle is now explicitly maker-first. Dual-sided BTC5 maker spread capture outranks directional wallet-copying until the repaired live path produces fresh submit/rest/cancel/executed evidence and the maker lane has its own validation surface. Use wallet flow as a confirmation overlay, not as the primary short-duration alpha thesis.

**Fast-wallet research note (2026-03-12):** New wallet research points to inventory-skewed spread capture and hybrid market making as the dominant public strategy family in Polymarket crypto fast markets. The strongest public clone targets are `vidarx` for a small BTC-only 5-minute bot and `gabagool22` / Arbigab for documented implementation patterns. Treat `k9Q2mX4L8A7ZP3R` as the large-scale hybrid reference wallet. Directional wallet copying remains secondary until fresh live maker evidence says otherwise.

**Maker-shadow reality check (2026-03-12):** The first corrected repo-native maker shadow run now exists at `reports/autoresearch/maker_shadow/latest.json`. It found `6` eligible BTC 5-minute markets, but `0` candidates under the conservative `YES + NO <= 0.97` cap. All sampled markets were sitting at `combined_bid_cost = 0.99`, so the lane is strategically promising but not yet economically validated on current books. At `0.99`, all `6` become shadow candidates, but that is only a thin `~1%` gross spread before queue loss, timeout scratches, and adverse selection. Keep maker spread capture shadow-only until fill-probability and scratch-loss validation exist.
...
### Fast-Market Queue Reset (2026-03-12)

| Priority | Program | Status | Gate |
|---|---|---|---|
| 1 | Dual-Sided BTC5 Maker Spread Capture | PRIMARY | Post-only, BTC-only, shadow-only until its own validation surface exists |
| 2 | Maker Fill / Toxicity / Queue Modeling | PRIMARY | Publish fill-probability and cancel/hold artifacts before any live tranche |
| 3 | Post-Fix BTC5 Live Evidence Capture | REQUIRED | The first repaired windows must reach submit, rest, cancel_unfilled, or executed, or isolate one blocker |
| 4 | Smart Wallet Flow Directional Copying | SECONDARY | Use only as a confirmation overlay after maker shadow proof and budget reset |

**Maker-lane defaults for the next cycle:** combined entry cost `YES + NO <= 0.97`, maker-only post-only quotes, BTC 5-minute markets only, `$5-$10` total notional per market, `6-8` concurrent markets max, and `120s` timeout/scratch discipline for unmatched hedges.

**Maker-lane threshold note:** `0.97` remains the conservative default. `0.99` is now an explicit sensitivity probe, not a live threshold. Promote above `0.97` only with measured maker fill-rate, queue/toxicity evidence, and scratch-loss accounting.

**Instance 6 queue contract (2026-03-12):** The next autoresearch cycle is graded on mirror-wallet outperformance benchmarks, not directional copying throughput. Required outputs each cycle: combined-cost distribution by market, dual-sided inventory overlap vs mirror-wallet cohort, fill-to-scratch loss ratio, maker candidate availability at `0.97`/`0.98`/`0.99`, and directional conversion deltas for live `d0_00075` vs shadow `d0_00150` on matched windows. Keep mutation-led local search frozen until budget reset unless a new live blocker shows a true operator miss.

Local command-node and BTC5 market mutation loops are budget exhausted today. Do not spend live-priority queue slots on budget-fallback churn until fresh live evidence exists.

## BUILDING — STRUCTURAL ALPHA (Parallel Sprint, 2026-03-07)

| # | Strategy | Modules | Tests | Status |
|---|----------|---------|-------|--------|
| SA-1 | A-6 Guaranteed Dollar Scanner | constraint_arb_engine, sum_violation_scanner, a6_sum_scanner, a6_executor, neg_risk_inventory, resolution_normalizer, signals/sum_violation/guaranteed_dollar.py | 223 total | **PARKED (2026-03-11, Cycle 3).** `kill_decision=kill` from `arb_empirical_snapshot.json`: upper_confidence_bound=0.0583 < 0.50 over 62 cycles. `fill_probability_gate=insufficient_data`, `settlement_path_gate=untested`. Kill-if-unchanged deadline 2026-03-14T18:49:58+00:00 remains active. No live-priority slot allocation this cycle. Reopen only with new fill-probability and settlement evidence. |
| SA-2 | B-1 Templated Dependency Engine | dependency_graph, relation_classifier, b1_executor, b1_monitor, relation_cache, bot/b1_template_engine.py | See above | **PARKED (2026-03-11, Cycle 3).** Zero deterministic template pairs in 1,000-market audit. Classification accuracy and false-positive rate unmeasured. Kill-if-unchanged deadline 2026-03-14T18:49:58+00:00 remains active. No live-priority slot allocation this cycle. Reopen only after 50-pair gold set validated at >=85% precision. |
...
|---------|------------|------|----------|-------------|---------|-------|
| 1 | A-6 | Guaranteed Dollar Scanner | 45% | 4.2 | **PARKED** | **Parked Cycle 3 (2026-03-11).** kill_decision=kill. fill_probability_gate=insufficient_data, settlement_path_gate=untested. No live-priority slot this cycle. Reopen with new fill-probability and settlement evidence only. |
| 2 | B-1 | Templated Dependency Engine | 45% | 4.1 | **PARKED** | **Parked Cycle 3 (2026-03-11).** 0 deterministic template pairs in 1000-market audit. No live-priority slot this cycle. Reopen after 50-pair gold set validated at >=85% precision. |
...
**How to Detect Signal in Code:**
```python
# Already partially implemented in resolution_rule_edge_playbook.md
for market in open_markets:
