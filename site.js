...
async function loadSiteData() {
  const [
    ...
    fetchFirstJson(['/reports/improvement_velocity.json', '/improvement_velocity.json']),
    fetchJson('/inventory/data/systems.json'),
    fetchJson('/reports/arb_empirical_snapshot.json'),
    fetchFirstJson([
      '/reports/nontrading_public_report.json',
      '/reports/jjn_public_report.json',
  ...
  const rawFundClaimStatus = String(pick(scoreboard, ['fund_realized_arr_claim_status'], 'blocked')).toLowerCase();
  const fundClaimStatus = launchBlocked ? 'blocked' : (rawFundClaimStatus || 'blocked');
  const fundClaimReason = launchBlocked
    ? 'Fund-level public claims stay blocked until runtime posture, wallet reconciliation, and capital attribution are consistent.'
    : pick(
      scoreboard,
  ...
  const elasticWorkerFamilyCoverage = `Trading proof lane + JJ-N (${jjnOfferName}) + checked-in publishing loop`;
  const elasticArtifactBackedProof = `${formatNumber(strategyCatalog.total, 0)} tracked strategies, ${formatNumber(dispatchWorkOrders, 0)} active work-orders, ${formatNumber(verificationBaselineTotal, 0)} checked tests, and JJ-N reports keep the story anchored to sanitized checked-in artifacts.`;
  ...
  return {
    ...
    runtimeSplitSummary: btc5LiveFilledRows > 0
      ? `BTC5 live proof / fund claim ${humanizeClaimStatus(fundClaimStatus).toLowerCase()} / JJ-N launch prep`
      : `launch ${launchBlocked ? 'blocked' : 'clear'} / paper mode default`,
    headlineSummary: launchBlocked
      ? 'Trading remains the live proof lane, but fund-level claims stay blocked while reconciliation and attribution are still open.'
      : 'Trading remains the live proof lane with explicit claim labels and paper-mode-safe contribution paths.',
    fundClaimStatusLabel: humanizeClaimStatus(fundClaimStatus),
    fundClaimReason,
