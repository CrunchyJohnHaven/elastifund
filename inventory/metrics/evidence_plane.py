...
class BenchmarkEvidencePacket:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "expected_arr_delta", _round_optional(_coalesce_float(self.expected_arr_delta)))
        object.__setattr__(
            self,
            "improvement_velocity",
            _round_optional(_coalesce_float(self.improvement_velocity)),
        )
        object.__setattr__(
            self,
            "candidate_confidence",
            _round_optional(_coalesce_float(self.candidate_confidence)),
        )
        object.__setattr__(
            self,
            "data_timestamp",
            None if self.data_timestamp is None else str(self.data_timestamp).strip() or None,
        )
        object.__setattr__(
            self,
            "pipeline_version",
            None if self.pipeline_version is None else str(self.pipeline_version).strip() or None,
        )
        object.__setattr__(self, "metadata", dict(self.metadata))
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "expected_arr_delta": self.expected_arr_delta,
