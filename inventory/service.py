...
def bot_detail_payload(bot_id: str) -> dict[str, Any]:
    system = get_system(bot_id)
    if system is None:
        raise LookupError(f"unknown bot: {bot_id}")
    related_runs = [_enrich_run(run) for run in list_runs() if run["system_id"] == bot_id]
...
def runs_payload(bot_id: str | None = None, status: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "methodology_version": BENCHMARK_SPEC_VERSION,
        "filters": {"bot_id": bot_id, "status": status},
        "total": len(runs),
        "items": runs,
    }
...
def paper_status_payload(bot_id: str | None = None) -> dict[str, Any]:
    runs = list_runs()
    if bot_id:
        runs = [run for run in runs if run["system_id"] == bot_id]
