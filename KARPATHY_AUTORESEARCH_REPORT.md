...
- Calibration lane: `bot/adaptive_platt.py`, `src/confidence_calibration.py`, `docs/strategy/llm-probability-calibration-system.md`
...
The forecasting and calibration lane is especially promising because Brier score, calibration error, and held-out validation are naturally benchmarkable. `bot/adaptive_platt.py` and `docs/strategy/llm-probability-calibration-system.md` are the clearest local analogues to Karpathy's fixed `val_bpb` target.
