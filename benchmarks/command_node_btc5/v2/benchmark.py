"""Frozen evaluator wrapper for the BTC5 command-node benchmark v2 lane."""
...
def _benchmark_id(manifest_path_value: str | Path) -> str:
    ...
    try:
        ...
        return "command_node_btc5_v2"
    return str(manifest.get("benchmark_id") or "command_node_btc5_v2")
    ...
    *,
    ...
    try:
        ...
        if message.startswith(legacy_prefix):
            ...
            raise ValueError(
                message.replace("command_node_btc5_v1", benchmark_id, 1)
            ) from error
        raise
