...
def walk_forward_model_competition(
    ...
    bootstrap = BootstrapEnsembleClassifier(
        feature_cols,
        members=15,
        sample_ratio=0.8,
        min_rows=min(80, max(20, len(train) // 2)),
        seed=model_seed,
    )
    bootstrap_predictions = bootstrap.predict_distribution(
        train,
        [int(row["label_up"]) for row in train],
        test,
    )
    bootstrap_probs = [item.mean_prob for item in bootstrap_predictions]
    bootstrap_avg_std = (
        statistics.mean(item.std_prob for item in bootstrap_predictions)
        if bootstrap_predictions
        else 0.0
    )
    bootstrap_avg_consensus = (
        statistics.mean(item.consensus_fraction for item in bootstrap_predictions)
        if bootstrap_predictions
        else 0.0
    )
    ...
    rows = [
        ...
        ("Historical resample", resample_probs),
        ("XGBoost", xgb_probs),
    ]
        ...
        table.append(
            {
                ...
                "avg_uncertainty": bootstrap_avg_std if name == "Bootstrap ensemble" else None,
                "avg_consensus": bootstrap_avg_consensus if name == "Bootstrap ensemble" else None,
            }
        )
