...
class FeatureEngineer:
    ...
    def _sampled_series(
        ...
        *,
        ...
        probe = ts - window_sec
        while probe <= ts:
            price = FeatureEngineer._nearest_price(probe, btc_ts, btc_prices)
    ...
    def _sma(series: list[float], window: int) -> float:
        if not series:
            return 0.0
        lookback = series[-window:] if len(series) >= window else series
        return statistics.fmean(lookback) if lookback else 0.0

    @staticmethod
    def _ema_series(series: list[float], span: int) -> list[float]:
        if not series:
            return []
    ...
    def _ema(series: list[float], span: int) -> float:
        ema_series = FeatureEngineer._ema_series(series, span)
    ...
    def _rsi(series: list[float], period: int = 14) -> float:
        ...
        diffs = diffs[-period:] if len(diffs) > period else diffs
        if not diffs:
            return 50.0
        gains = sum(max(delta, 0.0) for delta in diffs) / len(diffs)
        losses = sum(max(-delta, 0.0) for delta in diffs) / len(diffs)
        if losses <= 1e-12:
    ...
    def _macd(
        ...
        if len(series) < 2:
            return 0.0, 0.0, 0.0
        fast = FeatureEngineer._ema_series(series, fast_span)
        slow = FeatureEngineer._ema_series(series, slow_span)
        macd_series = [fast_val - slow_val for fast_val, slow_val in zip(fast, slow, strict=False)]
        signal_series = FeatureEngineer._ema_series(macd_series, signal_span)
        if not macd_series or not signal_series:
            return 0.0, 0.0, 0.0
        scale = max(abs(series[-1]), 1e-8)
        line = macd_series[-1] / scale
        signal = signal_series[-1] / scale
        hist = line - signal
        return line, signal, hist

    @staticmethod
    def _bollinger(series: list[float], period: int = 20) -> tuple[float, float]:
        if not series:
            return 0.0, 0.0
        ...
        if std <= 1e-12 or mean <= 0.0:
            return 0.0, 0.0
        zscore = (window[-1] - mean) / std
        bandwidth = (4.0 * std) / mean
        return zscore, bandwidth

    @staticmethod
    def _safe_return(current: float | None, previous: float | None) -> float:
    ...
    def build_feature_bundle(self, lookback_hours: int = 72) -> FeatureBundle:
        ...
        for row in prices:
            ...
            sampled_30m = self._sampled_series(ts, btc_ts, btc_price_vals, window_sec=1800, step_sec=60)
            sampled_2h = self._sampled_series(ts, btc_ts, btc_price_vals, window_sec=7200, step_sec=60)

            ma_5m = self._sma(sampled_30m, 5)
            ma_15m = self._sma(sampled_30m, 15)
            ma_30m = self._sma(sampled_30m, 30)
            ema_15m = self._ema(sampled_30m, 15)
            rsi_14 = self._rsi(sampled_30m, period=14)
            macd_line, macd_signal, macd_hist = self._macd(sampled_2h)
            bollinger_zscore_20, bollinger_bandwidth_20 = self._bollinger(sampled_30m, period=20)

            momentum_5m = self._safe_return(current_price, self._nearest_price(ts - 300, btc_ts, btc_price_vals))
            momentum_15m = self._safe_return(current_price, self._nearest_price(ts - 900, btc_ts, btc_price_vals))
            momentum_30m = self._safe_return(current_price, self._nearest_price(ts - 1800, btc_ts, btc_price_vals))
            ma_gap_5m = self._safe_return(current_price, ma_5m)
            ma_gap_15m = self._safe_return(current_price, ma_15m)
            ma_gap_30m = self._safe_return(current_price, ma_30m)
            ema_gap_15m = self._safe_return(current_price, ema_15m)
            vol_ratio_30m_2h = vol_30m / max(vol_2h, 1e-6) if vol_30m > 0.0 else 0.0
