...
from scripts.btc5_runtime_helpers import (
...
from scripts.research_artifacts import (
    utc_now as _shared_utc_now,
    utc_stamp as _shared_utc_stamp,
    write_json_and_markdown as _shared_write_json_and_markdown,
)
from scripts.btc5_monte_carlo_markdown import render_monte_carlo_markdown
from scripts.research_runtime import cap_for_mode, normalize_mode
from scripts.research_cli import add_mode_argument
...
DEFAULT_CAPACITY_TRADE_SIZES = (10.0, 20.0, 50.0, 100.0, 300.0)
...
LIVE_STAGE_FILL_RETENTION_THRESHOLDS = {1: 0.45, 2: 0.25, 3: 0.15}
LIVE_STAGE_LOSS_LIMIT_CEILINGS = {1: 0.35, 2: 0.45, 3: 0.55}
LIVE_STAGE_NON_POSITIVE_CEILINGS = {1: 0.45, 2: 0.50, 3: 0.55}
LIVE_STAGE_P95_DRAWDOWN_MULTIPLIERS = {1: 12.0, 2: 18.0, 3: 25.0}
SHADOW_FILL_RETENTION_FLOOR = 0.10
SHADOW_LOSS_LIMIT_CEILING = 0.90
SHADOW_NON_POSITIVE_CEILING = 0.90
SHADOW_P95_DRAWDOWN_MULTIPLIER = 40.0
MIN_DECISION_READY_LIVE_FILLS = 12
...
REGIME_SAMPLING_DIMENSIONS = ["session_name", "direction", "price_bucket", "delta_bucket"]
...
def _tail_risk_summary_from_payload(
    payload: dict[str, Any],
    *,
    non_positive_key: str = "non_positive_probability",
    daily_loss_key: str = "daily_loss_hit_probability",
    p95_drawdown_key: str = "p95_max_drawdown_usd",
    capital_efficiency_key: str = "capital_efficiency",
    active_trade_key: str = "expected_active_trade_load",
    loss_cluster_key: str = "loss_cluster_shock_hit_probability",
) -> dict[str, Any]:
    return _round_metrics(
        {
            "non_positive_path_probability": _safe_float(payload.get(non_positive_key), 0.0),
            "daily_loss_hit_probability": _safe_float(payload.get(daily_loss_key), 0.0),
            "p95_drawdown_usd": _safe_float(payload.get(p95_drawdown_key), 0.0),
            "capital_efficiency": _safe_float(payload.get(capital_efficiency_key), 0.0),
            "active_trade_load": _safe_float(payload.get(active_trade_key), 0.0),
            "loss_cluster_shock_hit_probability": _safe_float(payload.get(loss_cluster_key), 0.0),
        }
    )
...
def assemble_observed_rows(
    *,
    ...
    if refresh_remote:
    ...
    elif remote_cache_json.exists():
        cached_rows_payload = json.loads(remote_cache_json.read_text())
        if isinstance(cached_rows_payload, dict):
            cached_rows_iterable = cached_rows_payload.get("rows") or []
        elif isinstance(cached_rows_payload, list):
            cached_rows_iterable = cached_rows_payload
        else:
            cached_rows_iterable = []
        cached_rows = [
            _normalize_row(dict(row), source="remote_probe:cache")
            for row in cached_rows_iterable
        ...
        if cached_rows:
            ...
            sources.append(
                {
                    "path": str(remote_cache_json),
                    "source": "remote_probe:cache",
                    "rows_loaded": len(cached_rows),
                }
            )
    ...
    *,
    ...
    if not entries:
        empty = {
            ...
            "non_positive_path_probability": 1.0,
        ...
        empty["tail_risk_summary"] = _tail_risk_summary_from_payload(
            empty,
            active_trade_key="active_trade_load",
        )
        return empty
    ...
    summary = _round_metrics(
        {
            ...
            "non_positive_path_probability": 1.0 - profit_probability,
            ...
            "p95_drawdown_usd": _percentile(max_drawdowns, 95),
            ...
            "active_trade_load": avg_active_trades,
    ...
    summary["tail_risk_summary"] = _tail_risk_summary_from_payload(
        summary,
        active_trade_key="active_trade_load",
    )
    return summary
...
def run_monte_carlo(
    ...
    *,
    ...
    return _run_monte_carlo_from_entries(
        ...
        sampling_dimensions=REGIME_SAMPLING_DIMENSIONS,
        loss_cluster_scenarios=loss_cluster_scenarios,
    )


def _execution_drag_summary_from_sweep(
    sweep: dict[str, Any],
    *,
    baseline_execution_drag: dict[str, Any],
) -> dict[str, Any]:
    fill_retention_ratio = _safe_float(sweep.get("expected_fill_retention_ratio"), 0.0)
    expected_order_failed_probability = _safe_float(sweep.get("expected_order_failed_probability"), 0.0)
    expected_cancelled_unfilled_probability = _safe_float(
        sweep.get("expected_cancelled_unfilled_probability"),
        0.0,
    )
    ...
    baseline_order_failed_rate = _safe_float(baseline_execution_drag.get("order_failed_rate"), 0.0)
    baseline_cancelled_unfilled_rate = _safe_float(
        baseline_execution_drag.get("cancelled_unfilled_rate"),
        0.0,
    )
    baseline_post_only_retry_failure_rate = _safe_float(
        baseline_execution_drag.get("post_only_retry_failure_rate"),
        0.0,
    )
    return _round_metrics(
        {
            "same_level_fill_ratio": _safe_float(sweep.get("expected_same_level_fill_ratio"), 0.0),
            "fill_probability": _safe_float(sweep.get("expected_fill_probability"), 0.0),
            "fill_retention_ratio": fill_retention_ratio,
            "fill_retention_drag": max(0.0, 1.0 - fill_retention_ratio),
            "one_tick_worse_fill_ratio": _safe_float(
                sweep.get("expected_one_tick_worse_fill_ratio"),
                0.0,
            ),
            "one_tick_fill_probability": _safe_float(
                sweep.get("expected_one_tick_fill_probability"),
                0.0,
            ),
            "order_failed_probability": expected_order_failed_probability,
            "order_failed_rate_drag": max(0.0, expected_order_failed_probability - baseline_order_failed_rate),
            "cancelled_unfilled_probability": expected_cancelled_unfilled_probability,
            "cancelled_unfilled_rate_drag": max(
                0.0,
                expected_cancelled_unfilled_probability - baseline_cancelled_unfilled_rate,
            ),
            "post_only_retry_failure_rate": expected_post_only_retry_failure_rate,
            "post_only_retry_failure_rate_drag": max(
                0.0,
                expected_post_only_retry_failure_rate - baseline_post_only_retry_failure_rate,
            ),
            "one_tick_cost_per_active_fill_usd": _safe_float(
                sweep.get("expected_one_tick_cost_per_active_fill_usd"),
                0.0,
            ),
            "one_tick_cost_per_attempt_usd": _safe_float(
                sweep.get("expected_one_tick_cost_per_attempt_usd"),
                0.0,
            ),
        }
    )
...
def _stage_stress_entry(
    *,
    stage_key: str,
    capital_stage: int,
    sweep: dict[str, Any],
    baseline_execution_drag: dict[str, Any],
    loss_cluster_scenarios: list[dict[str, Any]],
) -> dict[str, Any]:
    execution_drag_summary = _execution_drag_summary_from_sweep(
        sweep,
        baseline_execution_drag=baseline_execution_drag,
    )
    tail_risk_summary = _tail_risk_summary_from_payload(
        sweep,
        non_positive_key="expected_non_positive_path_probability",
        daily_loss_key="expected_daily_loss_hit_probability",
        p95_drawdown_key="expected_p95_max_drawdown_usd",
        capital_efficiency_key="expected_capital_efficiency",
        active_trade_key="expected_active_trade_load",
        loss_cluster_key="expected_loss_cluster_shock_hit_probability",
    )
    return {
        "stage_key": stage_key,
        "stage_label": stage_key,
        "capital_stage": int(capital_stage),
        "trade_size_usd": _safe_float(sweep.get("trade_size_usd"), 0.0),
        "size_multiple": _safe_float(sweep.get("size_multiple"), 0.0),
        "sizing_track": str(sweep.get("sizing_track") or "unknown"),
        "sampling_dimensions": list(sweep.get("sampling_dimensions") or REGIME_SAMPLING_DIMENSIONS),
        "execution_drag_summary": execution_drag_summary,
        "tail_risk_summary": tail_risk_summary,
        "session_size_sensitivity": list(sweep.get("session_size_sensitivity") or []),
        "session_tail_contribution": list(sweep.get("session_tail_contribution") or []),
        "loss_cluster_scenarios": list(loss_cluster_scenarios)[:8],
    }


def _live_stage_gate_passed(sweep: dict[str, Any], *, capital_stage: int) -> bool:
    fill_threshold = LIVE_STAGE_FILL_RETENTION_THRESHOLDS.get(int(capital_stage), 0.15)
    loss_limit_ceiling = LIVE_STAGE_LOSS_LIMIT_CEILINGS.get(int(capital_stage), 0.55)
    return bool(
        _safe_float(sweep.get("expected_fill_retention_ratio"), 0.0) >= fill_threshold
        and _safe_float(sweep.get("expected_p05_arr_pct"), 0.0) > 0.0
        and _safe_float(sweep.get("expected_loss_limit_hit_probability"), 0.0) <= loss_limit_ceiling
    )


def _shadow_gate_passed(sweep: dict[str, Any]) -> bool:
    return bool(
        _safe_float(sweep.get("expected_fill_retention_ratio"), 0.0) >= SHADOW_FILL_RETENTION_FLOOR
        and _safe_float(sweep.get("expected_p05_arr_pct"), 0.0) > 0.0
        and _safe_float(sweep.get("expected_loss_limit_hit_probability"), 0.0) <= SHADOW_LOSS_LIMIT_CEILING
    )


def _blocking_categories_for_sweep(
    *,
    sweep: dict[str, Any],
    historical_replay_live_filled_rows: int,
    fill_threshold: float,
    loss_limit_ceiling: float,
    non_positive_ceiling: float,
    p95_drawdown_multiplier: float,
) -> tuple[list[str], list[str]]:
    categories: list[str] = []
    reasons: list[str] = []
    trade_size_usd = max(0.0, _safe_float(sweep.get("trade_size_usd"), 0.0))
    fill_retention_ratio = _safe_float(sweep.get("expected_fill_retention_ratio"), 0.0)
    retry_failure_rate = _safe_float(sweep.get("expected_post_only_retry_failure_rate"), 0.0)
    order_failed_rate_drag = _safe_float(sweep.get("order_failed_rate_drag"), 0.0)
    cancelled_unfilled_rate_drag = _safe_float(sweep.get("cancelled_unfilled_rate_drag"), 0.0)
    loss_limit_hit_probability = _safe_float(sweep.get("expected_loss_limit_hit_probability"), 0.0)
    daily_loss_hit_probability = _safe_float(sweep.get("expected_daily_loss_hit_probability"), 0.0)
    non_positive_path_probability = _safe_float(sweep.get("expected_non_positive_path_probability"), 0.0)
    p95_drawdown_usd = _safe_float(sweep.get("expected_p95_max_drawdown_usd"), 0.0)
    p05_arr_pct = _safe_float(sweep.get("expected_p05_arr_pct"), 0.0)

    if historical_replay_live_filled_rows < MIN_DECISION_READY_LIVE_FILLS:
        categories.append("missing_truth")
        reasons.append(
            f"replay_live_filled_rows_below_{MIN_DECISION_READY_LIVE_FILLS}:{historical_replay_live_filled_rows}"
        )
    if fill_retention_ratio < fill_threshold:
        categories.append("liquidity")
        reasons.append(
            f"fill_retention_ratio_below_threshold:{fill_retention_ratio:.4f}<{fill_threshold:.4f}"
        )
    retry_failure_ceiling = max(0.25, 1.0 - fill_threshold)
    if (
        retry_failure_rate > retry_failure_ceiling
        or order_failed_rate_drag > 0.05
        or cancelled_unfilled_rate_drag > 0.05
    ):
        categories.append("execution_quality")
        reasons.append(
            "post_only_retry_or_order_failed_drag_above_threshold"
        )
    p95_drawdown_ceiling = max(
        DEFAULT_LOSS_LIMIT_USD * 2.0,
        trade_size_usd * p95_drawdown_multiplier,
    )
    if (
        p05_arr_pct <= 0.0
        or loss_limit_hit_probability > loss_limit_ceiling
        or daily_loss_hit_probability > loss_limit_ceiling
        or non_positive_path_probability > non_positive_ceiling
        or p95_drawdown_usd > p95_drawdown_ceiling
    ):
        categories.append("drawdown_tails")
        if p05_arr_pct <= 0.0:
            reasons.append("expected_p05_arr_pct_non_positive")
        if loss_limit_hit_probability > loss_limit_ceiling:
            reasons.append(
                f"loss_limit_hit_probability_above_ceiling:{loss_limit_hit_probability:.4f}>{loss_limit_ceiling:.4f}"
            )
        if daily_loss_hit_probability > loss_limit_ceiling:
            reasons.append(
                f"daily_loss_hit_probability_above_ceiling:{daily_loss_hit_probability:.4f}>{loss_limit_ceiling:.4f}"
            )
        if non_positive_path_probability > non_positive_ceiling:
            reasons.append(
                "non_positive_path_probability_above_ceiling:"
                f"{non_positive_path_probability:.4f}>{non_positive_ceiling:.4f}"
            )
        if p95_drawdown_usd > p95_drawdown_ceiling:
            reasons.append(
                f"p95_drawdown_usd_above_ceiling:{p95_drawdown_usd:.4f}>{p95_drawdown_ceiling:.4f}"
            )
    return _unique_strs(categories), _unique_strs(reasons)


def _evidence_required_from_categories(categories: list[str]) -> list[str]:
    evidence: list[str] = []
    if "missing_truth" in categories:
        evidence.append(
            f"Accumulate at least {MIN_DECISION_READY_LIVE_FILLS} replayable live-filled BTC5 rows for this profile."
        )
    if "liquidity" in categories:
        evidence.append("Improve fill retention at the target ticket size with session-aware concentration.")
    if "execution_quality" in categories:
        evidence.append("Reduce post-only retry failures and order-failed drag at the target ticket size.")
    if "drawdown_tails" in categories:
        evidence.append("Improve stressed tail metrics: P05 ARR, daily-loss-hit probability, and P95 drawdown.")
    return evidence


def _evidence_verdict_payload(
    *,
    gate_passed: bool,
    ...
    category_to_true_negative = {
        "liquidity": "fill_retention_below_threshold",
        "execution_quality": "post_only_execution_quality_below_threshold",
        "drawdown_tails": "tail_risk_below_threshold",
    }
    true_negative_items = _unique_strs(
        [
            category_to_true_negative[str(category)]
            for category in blocking_categories
            if str(category) in category_to_true_negative
        ]
    )

    if gate_passed:
        evidence_verdict = (
            "tentatively_positive_but_underpowered"
            if missing_evidence_items
            else "sufficient_positive_evidence"
        )
    elif missing_evidence_items and true_negative_items:
        evidence_verdict = "mixed_missing_and_true_negative"
    elif missing_evidence_items:
        evidence_verdict = "missing_evidence_only"
    elif true_negative_items:
    ...
    return {
        ...
        "true_negative_items": true_negative_items,
    }


def build_capital_ladder_summary(
    *,
    profile_name: str,
    historical: dict[str, Any],
    reference_trade_size_usd: float,
    current_base_sweep: dict[str, Any] | None,
    stage_sweeps: list[dict[str, Any]],
    shadow_sweeps: list[dict[str, Any]],
) -> dict[str, Any]:
    historical_replay_live_filled_rows = max(
        0,
        _safe_int(historical.get("replay_live_filled_rows")),
    )
    live_stage_decisions: list[dict[str, Any]] = []
    candidate_sweeps: list[tuple[str, int, dict[str, Any]]] = []
    if isinstance(current_base_sweep, dict):
        candidate_sweeps.append(("current", 0, current_base_sweep))
    for sweep in stage_sweeps:
        if not isinstance(sweep, dict):
            continue
        candidate_sweeps.append(
            (
                str(sweep.get("stage_label") or f"stage_{_safe_int(sweep.get('capital_stage'))}"),
                _safe_int(sweep.get("capital_stage")),
                sweep,
            )
        )

    max_live_stage = -1
    safe_live_trade_size_usd = 0.0
    safe_live_stage_label = "none"
    for stage_label, capital_stage, sweep in candidate_sweeps:
        gate_stage = max(1, int(capital_stage or 1))
        gate_passed = _live_stage_gate_passed(sweep, capital_stage=gate_stage)
        blocking_categories, blocking_reasons = ([], [])
        if not gate_passed:
            blocking_categories, blocking_reasons = _blocking_categories_for_sweep(
                sweep=sweep,
                historical_replay_live_filled_rows=historical_replay_live_filled_rows,
                fill_threshold=LIVE_STAGE_FILL_RETENTION_THRESHOLDS.get(gate_stage, 0.15),
                loss_limit_ceiling=LIVE_STAGE_LOSS_LIMIT_CEILINGS.get(gate_stage, 0.55),
                non_positive_ceiling=LIVE_STAGE_NON_POSITIVE_CEILINGS.get(gate_stage, 0.55),
                p95_drawdown_multiplier=LIVE_STAGE_P95_DRAWDOWN_MULTIPLIERS.get(gate_stage, 25.0),
            )
        decision = _round_metrics(
            {
                "stage_label": stage_label,
                "capital_stage": int(capital_stage),
                "trade_size_usd": _safe_float(sweep.get("trade_size_usd"), 0.0),
                "status": "live_ready" if gate_passed else "blocked_live",
                "deployment_class": "live" if gate_passed else "blocked",
                "gate_passed": gate_passed,
                "blocking_categories": blocking_categories,
                "blocking_reasons": blocking_reasons,
                "evidence_required": _evidence_required_from_categories(blocking_categories),
                "execution_drag_summary": dict(sweep.get("execution_drag_summary") or {}),
                "tail_risk_summary": dict(sweep.get("tail_risk_summary") or {}),
                "expected_fill_probability": _safe_float(sweep.get("expected_fill_probability"), 0.0),
                ...
                "expected_non_positive_path_probability": _safe_float(
                    sweep.get("expected_non_positive_path_probability"),
                    0.0,
                ),
                "expected_p95_max_drawdown_usd": _safe_float(
                    sweep.get("expected_p95_max_drawdown_usd"),
                    0.0,
                ),
                "expected_p05_arr_pct": _safe_float(sweep.get("expected_p05_arr_pct"), 0.0),
                **_evidence_verdict_payload(
                    gate_passed=gate_passed,
                    blocking_categories=blocking_categories,
                    historical_replay_live_filled_rows=historical_replay_live_filled_rows,
                ),
        ...
        live_stage_decisions.append(decision)
        if gate_passed and capital_stage >= max_live_stage:
            max_live_stage = int(capital_stage)
            safe_live_trade_size_usd = _safe_float(sweep.get("trade_size_usd"), 0.0)
            safe_live_stage_label = stage_label

    shadow_only: list[dict[str, Any]] = []
    for sweep in shadow_sweeps:
        if not isinstance(sweep, dict):
            continue
        gate_passed = _shadow_gate_passed(sweep)
        blocking_categories, blocking_reasons = ([], [])
        if not gate_passed:
            blocking_categories, blocking_reasons = _blocking_categories_for_sweep(
                sweep=sweep,
                historical_replay_live_filled_rows=historical_replay_live_filled_rows,
                fill_threshold=SHADOW_FILL_RETENTION_FLOOR,
                loss_limit_ceiling=SHADOW_LOSS_LIMIT_CEILING,
                non_positive_ceiling=SHADOW_NON_POSITIVE_CEILING,
                p95_drawdown_multiplier=SHADOW_P95_DRAWDOWN_MULTIPLIER,
            )
        shadow_only.append(
            _round_metrics(
                {
                    "shadow_label": str(sweep.get("shadow_label") or ""),
                    "trade_size_usd": _safe_float(sweep.get("trade_size_usd"), 0.0),
                    "status": "shadow_ready" if gate_passed else "shadow_blocked",
                    "deployment_class": "shadow_only" if gate_passed else "blocked",
                    "gate_passed": gate_passed,
                    "blocking_categories": blocking_categories,
                    "blocking_reasons": blocking_reasons,
                    "evidence_required": _evidence_required_from_categories(blocking_categories),
    ...
    shadow_ready_trade_sizes_usd = [
        _safe_float(item.get("trade_size_usd"), 0.0)
        for item in shadow_only
        if bool(item.get("gate_passed"))
    ]
    next_rung_floor_trade_size_usd = max(reference_trade_size_usd, safe_live_trade_size_usd)
    blocked_higher_notional = [
        item
        for item in (live_stage_decisions + shadow_only)
        if _safe_float(item.get("trade_size_usd"), 0.0) > next_rung_floor_trade_size_usd
        and not bool(item.get("gate_passed"))
    ]
    next_notional_gate = blocked_higher_notional[0] if blocked_higher_notional else None
    blocking_categories_summary: dict[str, int] = {}
    for item in blocked_higher_notional:
        for category in item.get("blocking_categories") or []:
            blocking_categories_summary[str(category)] = blocking_categories_summary.get(str(category), 0) + 1

    top_level_status = "shadow_only_higher_notional"
    if max_live_stage >= 2:
        top_level_status = "live_stage2_ready"
    elif max_live_stage >= 1:
        top_level_status = "live_stage1_improved"

    return {
        "metric_name": "capital_ladder_summary",
        ...
        "live_now": {
            "status": (
                safe_live_stage_label
                if safe_live_stage_label != "none"
                else "no_live_size_ready"
            ),
            "safe_trade_size_usd": round(safe_live_trade_size_usd, 4),
            "safe_stage_label": safe_live_stage_label,
            "max_live_stage": max(0, int(max_live_stage)),
            "live_ready_stage_labels": [
                str(item.get("stage_label") or "")
                for item in live_stage_decisions
                if bool(item.get("gate_passed"))
            ],
        },
        "live_stage_decisions": live_stage_decisions,
        "shadow_only": shadow_only,
        "shadow_ready_trade_sizes_usd": shadow_ready_trade_sizes_usd,
        "next_rung_floor_trade_size_usd": round(next_rung_floor_trade_size_usd, 4),
        "next_notional_gate": next_notional_gate,
        "blocking_categories_summary": blocking_categories_summary,
        "evidence_required_to_raise_notional": _evidence_required_from_categories(
            list((next_notional_gate or {}).get("blocking_categories") or [])
        ),
    }
...
def build_capacity_stress_summary(
    *,
    ...
    base_loss_cluster_shock_hit_probability = _safe_float(
        monte_carlo.get("loss_cluster_shock_hit_probability"),
        0.0,
    )
        ...
        execution_drag_summary = _execution_drag_summary_from_sweep(
            {
                **aggregate_sensitivity,
                "expected_fill_probability": aggregate_sensitivity.get("expected_fill_probability"),
                "expected_fill_retention_ratio": fill_retention_ratio,
                "expected_order_failed_probability": aggregate_sensitivity.get("expected_order_failed_probability"),
                "expected_cancelled_unfilled_probability": aggregate_sensitivity.get(
                    "expected_cancelled_unfilled_probability"
                ),
                "expected_post_only_retry_failure_rate": aggregate_sensitivity.get(
                    "expected_post_only_retry_failure_rate"
                ),
                "expected_one_tick_fill_probability": aggregate_sensitivity.get(
                    "expected_one_tick_fill_probability"
                ),
                "expected_one_tick_cost_per_active_fill_usd": aggregate_sensitivity.get(
                    "expected_one_tick_cost_per_active_fill_usd"
                ),
                "expected_one_tick_cost_per_attempt_usd": aggregate_sensitivity.get(
                    "expected_one_tick_cost_per_attempt_usd"
                ),
            },
            baseline_execution_drag=baseline_execution_drag,
            ...
            _round_metrics(
                {
                    ...
                    "fill_retention_drag": _safe_float(execution_drag_summary.get("fill_retention_drag"), 0.0),
                    "order_failed_rate_drag": _safe_float(
                        execution_drag_summary.get("order_failed_rate_drag"),
                        0.0,
                    ),
                    "cancelled_unfilled_rate_drag": _safe_float(
                        execution_drag_summary.get("cancelled_unfilled_rate_drag"),
                        0.0,
                    ),
                    "post_only_retry_failure_rate_drag": _safe_float(
                        execution_drag_summary.get("post_only_retry_failure_rate_drag"),
                        0.0,
                    ),
                    ...
                    "expected_loss_cluster_shock_hit_probability": _safe_float(
                        stress_monte_carlo.get("loss_cluster_shock_hit_probability"),
                        0.0,
                    ),
                    "loss_cluster_shock_hit_probability_impact": _safe_float(
                        stress_monte_carlo.get("loss_cluster_shock_hit_probability"),
                        0.0,
                    )
                    - base_loss_cluster_shock_hit_probability,
                    ...
                    "execution_drag_summary": execution_drag_summary,
                    "tail_risk_summary": tail_risk_summary,
                    "sampling_dimensions": list(stress_monte_carlo.get("sampling_dimensions") or REGIME_SAMPLING_DIMENSIONS),
        ...
        execution_drag_summary = _execution_drag_summary_from_sweep(
            {
                **aggregate_sensitivity,
                "expected_fill_probability": aggregate_sensitivity.get("expected_fill_probability"),
                "expected_fill_retention_ratio": aggregate_sensitivity.get("expected_fill_retention_ratio"),
                "expected_order_failed_probability": aggregate_sensitivity.get("expected_order_failed_probability"),
                "expected_cancelled_unfilled_probability": aggregate_sensitivity.get(
                    "expected_cancelled_unfilled_probability"
                ),
            ...
            _round_metrics(
                {
                    ...
                    "sampling_dimensions": list(
                        stress_monte_carlo.get("sampling_dimensions") or REGIME_SAMPLING_DIMENSIONS
                    ),
                    "session_tail_contribution": list(
                        stress_monte_carlo.get("session_tail_contribution") or []
                    ),
    ...
    stage_stress_entries: list[dict[str, Any]] = []
    if isinstance(current_base_sweep, dict):
        stage_stress_entries.append(
            _stage_stress_entry(
                stage_key="current",
                capital_stage=0,
                sweep=current_base_sweep,
                baseline_execution_drag=baseline_execution_drag,
                loss_cluster_scenarios=loss_cluster_scenarios,
            )
        )
    for stage_label, capital_stage in (("stage_1", 1), ("stage_2", 2), ("stage_3", 3)):
        sweep = next(
            (
                item
                for item in stage_sweeps
                if int(item.get("capital_stage") or 0) == capital_stage
            ),
            None,
        )
        if not isinstance(sweep, dict):
    ...
    stage_stress_summary = {
        "metric_name": "stage_stress_summary",
        "profile_name": profile.name,
        "reference_trade_size_usd": round(reference_trade_size_usd, 4),
        "loss_limit_usd": round(loss_limit_usd, 4),
        "stage_order": [str(item.get("stage_key") or "") for item in stage_stress_entries],
        "stages": stage_stress_entries,
        "stages_by_label": {
            str(item.get("stage_key") or ""): item
            for item in stage_stress_entries
            if str(item.get("stage_key") or "").strip()
        },
    }
    capital_ladder = build_capital_ladder_summary(
        profile_name=profile.name,
        historical=historical,
        reference_trade_size_usd=reference_trade_size_usd,
    ...
    return {
        ...
        "stage_size_caps_usd": {
            "current": round(reference_trade_size_usd, 4),
            **{
        ...
        "stage_sweeps": stage_sweeps,
        "shadow_sweeps": shadow_sweeps,
        "shadow_trade_size_assessments": list(capital_ladder.get("shadow_only") or []),
        "stage_stress_summary": stage_stress_summary,
...
def _render_markdown(summary: dict[str, Any]) -> str:
    return render_monte_carlo_markdown(summary)
...
def _write_outputs(
    ...
    *,
    ...
    _shared_write_json_and_markdown(
        json_path=json_path,
...
def build_summary(
    *,
    ...
    capital_ladder_summary = dict(
        ((capacity_stress_profiles.get(recommended_reference) or {}).get("capital_ladder") or {})
    )
    if capital_ladder_summary:
        capital_ladder_summary["recommended_reference"] = recommended_reference
    shadow_trade_size_assessments = list(
        ((capacity_stress_profiles.get(recommended_reference) or {}).get("shadow_trade_size_assessments") or [])
    )
    ...
    return {
        ...
        "capital_ladder_summary": capital_ladder_summary,
...
def parse_args() -> argparse.Namespace:
    ...
    add_mode_argument(
        parser,
        help_text=(
            "Execution mode: full runs full simulation; analyze/quick cap path/candidate count "
            "for a cheap local loop."
        ),
    )
