...
if [[ "${1:-}" == "--help" || "${1:-}" == "-h" ]]; then
  cat <<'EOF'
Usage: scripts/vps_setup.sh

One-shot setup for the Dublin VPS trading box.
Run this on the target VPS after the repo and .env are present at:
...
This script:
  ...
  5. installs or refreshes the systemd service
  6. installs a cron-based data sync
EOF
  exit 0
fi
