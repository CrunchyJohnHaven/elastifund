...
| Metadata | Value |
|---|---|
| Canonical file | `docs/REPO_MAP.md` |
| Role | Path ownership and safe edit boundaries |
...
| `docs/FORK_AND_RUN.md` | easiest paper-mode fork/bootstrap path |
...
| `docs/ops/finance_control_plane.md` | finance control-plane policy, env knobs, rollout gates, and artifact contract |
...
For posture and cycle status, prefer `reports/runtime_truth_latest.json`, `reports/remote_cycle_status.json`, `reports/remote_service_status.json`, and `research/edge_backlog_ranked.md` over stale prose. Existing JSON handoff artifacts remain the runtime contract.
...
| `nontrading/` | non-trading revenue automation plus finance control-plane code | separate lane from live trading; `nontrading/engines/` holds the five-engine Phase 0 stubs and `nontrading/finance/` holds personal-CFO logic |
...
| `build/`, `develop/`, `diary/`, `elastic/`, `leaderboards/`, `manage/`, `roadmap/` | static website route stubs, hidden onboarding surfaces, and leaderboard pages | keep public messaging aligned with `docs/numbered/11_PUBLIC_MESSAGING.md` and the numbered-doc lane |
...
- `docs/numbered/` is the canonical numbered-doc lane; if numbered root shims exist, keep them as redirects to canonical `docs/numbered/*` files.
...
- Finance imports stay outside source control; finance truth lives under `state/` via `JJ_FINANCE_DB_PATH`, and finance reports live under `reports/finance/`.
