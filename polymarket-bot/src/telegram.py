...
class TelegramNotifier:
    ...
    async def send_cooldown(self, consecutive_losses: int, cooldown_seconds: int) -> bool:
        """Send cooldown activation alert.
    ...
    async def send_fill_rate_report(
        self,
        total_orders: int,
        filled: int,
        cancelled: int,
        ...
        """Send fill rate stats report (every 20 cycles)."""
        ...
        msg = (
            f"📊 <b>Fill Rate Report</b>\n\n"
            f"<b>Total Orders:</b> {total_orders}\n"
            f"<b>Filled:</b> {filled} ({fill_rate:.1f}%)\n"
