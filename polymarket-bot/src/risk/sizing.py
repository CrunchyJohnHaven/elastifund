...
# Time-aware Kelly dampener (QR-PM-2026-0041: "NEVER full Kelly on 5min markets")
# Maps hours-to-resolution → Kelly multiplier cap.
# Short-horizon markets have higher variance → aggressively reduce Kelly.
RESOLUTION_KELLY_DAMPENER: list[tuple[float, float]] = [
    (0.5, 0.05),    # <30 min: 5% Kelly max (near-term = noise-dominated)
    (2.0, 0.10),    # <2 hours: 10% Kelly max
    (12.0, 0.20),   # <12 hours: 20% Kelly max
    (48.0, 0.50),   # <2 days: 50% Kelly max
    (168.0, 0.75),  # <1 week: 75% Kelly max
    (float("inf"), 1.0),  # Longer: no dampening
]
...
class SizingCaps:
    ...
    hours_to_resolution: Optional[float] = None  # For time-aware Kelly dampening
...
def resolution_kelly_cap(hours_to_resolution: Optional[float]) -> float:
    """Get Kelly multiplier cap based on time to market resolution.

    Short-horizon markets are noise-dominated and should use much smaller
    Kelly fractions. Per QR-PM-2026-0041: "NEVER full Kelly on 5min markets."

    Args:
        hours_to_resolution: Estimated hours until market resolves.
            None means unknown (no dampening applied).

    Returns:
        Maximum Kelly multiplier (0-1). Applied as a cap on effective_mult.
    """
    if hours_to_resolution is None:
        return 1.0

    for threshold, cap in RESOLUTION_KELLY_DAMPENER:
...
def position_usd(
    ...
    # 2b. Time-aware Kelly dampener (QR-PM-2026-0041)
    if caps is not None and caps.hours_to_resolution is not None:
        time_cap = resolution_kelly_cap(caps.hours_to_resolution)
        if effective_mult > time_cap:
            _log(
                "info",
                "time_kelly_dampened",
                hours=caps.hours_to_resolution,
                original_mult=round(effective_mult, 4),
                capped_mult=round(time_cap, 4),
            )
            effective_mult = time_cap
