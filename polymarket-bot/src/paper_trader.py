...
from src.resolution_estimator import velocity_adjusted_ev
...
class PaperTrader:
    ...
    def __init__(
        ...
        Args:
            ...
            max_days_to_resolution: Max days before capital lock-up penalty (default 14)
        """
    ...
    async def _scan_and_analyze(self) -> list[dict]:
        ...
        Applies capital velocity filter: signals from markets that lock up
        capital too long (beyond max_days_to_resolution) get penalized or
        blocked entirely.

        Returns:
            List of signal dicts with market info, trade recommendation,
            and capital velocity metadata.
        """
        signals = []
        ...
        # Build lookup: market_id → opportunity (carries resolution data)
        opp_lookup: dict[str, dict] = {}
        for opp in opportunities:
            opp_lookup[opp["market_id"]] = opp
            ...
            if markets_for_analysis:
                try:
                    ...
                    for r in results:
                        if r.get("mispriced"):
                            mid = r["market_id"]
                            opp = opp_lookup.get(mid, {})
                            est_days = opp.get("estimated_days", 14.0)
                            taker_fee = r.get("taker_fee", 0.0)
                            ...
                            signals.append({
                                ...
                                "resolution_bucket": opp.get("resolution_bucket", "1-4w"),
                                "taker_fee": taker_fee,
                            })
                except Exception as e:
                            ...
                            if high is not None and low is not None:
                                ...
                                if eval_result["signal"] != "hold":
                                    ...
                                    signals.append({
                                        ...
                                        "estimated_days": opp.get("estimated_days", 2.0),
                                        "resolution_bucket": opp.get("resolution_bucket", "1-3d"),
                                        "taker_fee": 0.0,
                                    })
                            break
        ...
        # 4. Apply capital velocity filter: compute adjusted EV and block/penalize
        filtered_signals = []
        blocked_count = 0
        for sig in signals:
            ev_info = velocity_adjusted_ev(
                edge=abs(sig.get("edge", 0)),
                estimated_days=sig.get("estimated_days", 14.0),
                taker_fee=sig.get("taker_fee", 0.0),
                max_days=self.max_days_to_resolution,
            )
            sig["velocity_score"] = ev_info["adjusted_ev"]
            sig["velocity_penalty"] = ev_info["penalty"]
            sig["velocity_blocked"] = ev_info["blocked"]

            if ev_info["blocked"]:
                blocked_count += 1
                logger.info(
                    "velocity_blocked",
                    market=sig.get("question", "")[:60],
                    estimated_days=sig.get("estimated_days"),
                    penalty=ev_info["penalty"],
                )
            else:
                filtered_signals.append(sig)

        # Sort remaining by velocity-adjusted EV descending
        filtered_signals.sort(key=lambda s: s.get("velocity_score", 0), reverse=True)

        if signals:
            logger.info(
                "velocity_filter_applied",
    ...
    async def _process_signal(self, signal: dict) -> None:
        ...
        # Capital velocity metadata
        est_days = signal.get("estimated_days", 14.0)
        vel_score = signal.get("velocity_score", 0.0)
        vel_penalty = signal.get("velocity_penalty", 1.0)
        ...
        trade = {
            ...
            "bankroll": round(bankroll, 2),
            "category": category,
            "category_haircut": sizing.category_haircut,
            "reasoning": signal.get("reasoning", "")[:300],
            "estimated_days": est_days,
            "velocity_score": round(vel_score, 2),
            "velocity_penalty": round(vel_penalty, 4),
            "resolution_bucket": signal.get("resolution_bucket", ""),
            "status": "simulated",
        }
        ...
        # Track in open positions (with capital lock-up accounting)
