...
# Calibration map from backtest (532 resolved markets, 2026-03-05)
# Maps Claude's raw estimate midpoint → observed actual YES rate
CALIBRATION_MAP = {
    0.05: 0.157,   # Claude says 0-10% → actual 15.7%
    0.15: 0.120,   # Claude says 10-20% → actual 12.0%
    0.25: 0.220,   # Claude says 20-30% → actual 22.0%
    0.35: 0.625,   # Claude says 30-40% → actual 62.5%
    0.45: 0.417,   # Claude says 40-50% → actual 41.7%
    0.55: 0.466,   # Claude says 50-60% → actual 46.6%
    0.65: 0.455,   # Claude says 60-70% → actual 45.5%
    0.75: 0.528,   # Claude says 70-80% → actual 52.8%
    0.85: 0.632,   # Claude says 80-90% → actual 63.2%
    0.95: 0.632,   # Claude says 90-100% → actual 63.2%
}
...
def calibrate_probability(raw_prob: float) -> float:
    """Apply temperature-scaling-inspired calibration to Claude's raw probability.

    Uses piecewise linear interpolation between observed calibration points
    from 532-market backtest. This corrects Claude's systematic YES overconfidence.
    """
