...
async def test_process_window_strong_validated_session_uses_full_stage_cap(
    ...
    assert result["sizing_target_usd"] == pytest.approx(10.0)
    ...
    assert row["sizing_target_usd"] == pytest.approx(10.0)
    assert row["sizing_cap_usd"] == pytest.approx(10.0)
...
async def test_process_window_balanced_hour_11_candidate_stays_standard_without_tight_delta(
    ...
    assert result["size_usd"] == pytest.approx(9.9969, rel=1e-4)
    assert result["sizing_target_usd"] == pytest.approx(10.0)
    assert result["sizing_cap_usd"] == pytest.approx(20.0)
