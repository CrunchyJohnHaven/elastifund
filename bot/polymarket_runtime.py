...
MarketScanner = import_polymarket_module("scanner").MarketScanner
ClaudeAnalyzer = import_polymarket_module("claude_analyzer").ClaudeAnalyzer

try:
    _telegram_module = import_polymarket_module("telegram")
except ImportError:
    TelegramNotifier = None
    TelegramBot = None
else:
    TelegramNotifier = getattr(_telegram_module, "TelegramNotifier", None)

    class TelegramBot:
        def __init__(self, *args, **kwargs):
            if TelegramNotifier is None:
                raise ImportError("TelegramNotifier is unavailable")
            self._notifier = TelegramNotifier(*args, **kwargs)
