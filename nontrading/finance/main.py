...
from nontrading.finance.policy import (
    ...
    load_json,
    load_trading_finance_policy_context,
...
def _finance_gate_snapshot(
    *,
    rollout_gates: Any,
    finance_gate_pass: bool,
    treasury_gate_pass: bool | None = None,
    capital_expansion_only_hold: bool | None = None,
    ...
    rollout_reason = next(
        (
            str(item).strip()
            for item in (getattr(rollout_gates, "reasons", ()) or ())
            if str(item).strip()
        ),
        None,
    )
    gate_reason = reason or rollout_reason or ("queue_ready" if finance_gate_pass else "rollout_gate_not_ready_for_live_treasury")
    return {
...
def _resolve_finance_gate_state(*, rollout_gates: Any, trading_finance_policy: dict[str, Any]) -> dict[str, Any]:
    baseline_live_trading_pass = bool(
        rollout_gates.ready_for_live_treasury and trading_finance_policy.get("baseline_live_allowed", True)
    )
    treasury_gate_pass = bool(
        rollout_gates.ready_for_live_treasury
        and trading_finance_policy.get("baseline_live_allowed", True)
        and not trading_finance_policy["hold_live_treasury"]
    )
    capital_expansion_only_hold = bool(
        baseline_live_trading_pass and trading_finance_policy["hold_live_treasury"]
    )
    return {
...
def run_sync(store: FinanceStore, settings: FinanceSettings) -> dict[str, Any]:
    ...
    gate_state = _resolve_finance_gate_state(
        rollout_gates=rollout_gates,
        trading_finance_policy=trading_finance_policy,
    )
    finance_gate_pass = gate_state["baseline_live_trading_pass"]
    report = {
        ...
        "baseline_live_trading_pass": gate_state["baseline_live_trading_pass"],
        "treasury_gate_pass": gate_state["treasury_gate_pass"],
        "capital_expansion_only_hold": gate_state["capital_expansion_only_hold"],
        "finance_state": trading_finance_policy["finance_state"],
        "capital_expansion_policy": trading_finance_policy,
        "finance_gate": _finance_gate_snapshot(
            rollout_gates=rollout_gates,
            finance_gate_pass=finance_gate_pass,
            treasury_gate_pass=gate_state["treasury_gate_pass"],
            capital_expansion_only_hold=gate_state["capital_expansion_only_hold"],
            stage_cap=trading_finance_policy["stage_cap"],
            reason=(
                trading_finance_policy["reason"]
                if trading_finance_policy["hold_live_treasury"]
                else next(iter(rollout_gates.reasons), None)
            ),
            remediation=trading_finance_policy["remediation"] if trading_finance_policy["hold_live_treasury"] else None,
        ),
...
def run_audit(store: FinanceStore, settings: FinanceSettings, queue: FinanceActionQueue | None = None) -> dict[str, Any]:
    ...
    if queue is not None:
        queued_actions = [
            ...
            and max(
                float(finding.get("monthly_cost_usd", 0.0) or 0.0),
                float(finding.get("estimated_savings_usd", 0.0) or 0.0),
            ) > 0.0
        ]
...
def run_allocate(store: FinanceStore, settings: FinanceSettings, queue: FinanceActionQueue) -> dict[str, Any]:
    ...
    action_queue_report = queue.build_report()
    runtime_truth = load_json(settings.workspace_root / "reports" / "runtime_truth_latest.json") or {}
    finance_latest = load_json(settings.latest_report_path) or {}
    model_budget_plan, model_budget_actions = build_model_budget_plan(
        finance_latest=finance_latest,
        action_queue=action_queue_report,
        runtime_truth=runtime_truth,
    )
    settings.model_budget_plan_path.write_text(
        json.dumps(model_budget_plan, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    store.record_snapshot("model_budget_plan", model_budget_plan["schema_version"], model_budget_plan)
...
def run_execute(store: FinanceStore, settings: FinanceSettings, queue: FinanceActionQueue, executor: FinanceExecutor, mode: str | None = None) -> dict[str, Any]:
    ...
    if selected_mode == "live_treasury" and trading_finance_policy["hold_live_treasury"] and not gate_state["baseline_live_trading_pass"]:
        live_hold = {
            "status": "hold_no_spend",
            "state": trading_finance_policy["finance_state"],
            "requested_mode": selected_mode,
            "reason": trading_finance_policy["reason"],
            "policy_checks": policy_checks,
            "retry_in_minutes": 30,
            "retry_at": _plus_minutes(utc_now(), 30),
            "remediation": trading_finance_policy["remediation"],
            "block_reasons": trading_finance_policy["block_reasons"],
            "stage_cap": trading_finance_policy["stage_cap"],
        }
        result = {
        ...
        latest["generated_at"] = utc_now()
        latest["rollout_gates"] = rollout_gates.to_dict()
        latest["status_snapshot"] = store.status_snapshot()
        latest["totals"] = build_finance_totals(store, settings)
        latest["finance_gate_pass"] = False
        latest["baseline_live_trading_pass"] = False
        latest["treasury_gate_pass"] = False
        latest["capital_expansion_only_hold"] = False
        latest["finance_state"] = trading_finance_policy["finance_state"]
        latest["capital_expansion_policy"] = trading_finance_policy
        latest["finance_gate"] = _finance_gate_snapshot(
            rollout_gates=rollout_gates,
            finance_gate_pass=False,
            treasury_gate_pass=False,
            ...
            reason=trading_finance_policy["reason"],
            remediation=trading_finance_policy["remediation"],
            retry_at=live_hold["retry_at"],
            retry_in_minutes=live_hold["retry_in_minutes"],
            policy_checks=policy_checks,
        )
    ...
    if fallback_action is not None and selected_mode in {"live_spend", "live_treasury"}:
        ...
        if preflight_reason and (
            ...
            latest["finance_gate"] = _finance_gate_snapshot(
                ...
                requested_mode=selected_mode,
                reason=preflight_reason,
                remediation=result["live_hold"]["remediation"],
                retry_at=retry_at,
                retry_in_minutes=30,
                destination=fallback_action.destination,
                policy_checks=policy_checks,
    ...
    try:
        ...
        result["finance_gate_pass"] = gate_state["baseline_live_trading_pass"]
        result["treasury_gate_pass"] = gate_state["treasury_gate_pass"]
        result["finance_state"] = trading_finance_policy["finance_state"]
        if fallback_action is not None:
    ...
    latest["status_snapshot"] = store.status_snapshot()
    latest["totals"] = build_finance_totals(store, settings)
    finance_gate_pass = bool(result["finance_gate_pass"])
    live_hold = result.get("live_hold") if isinstance(result.get("live_hold"), dict) else {}
    treasury_gate_pass = bool(result.get("treasury_gate_pass", False))
    capital_expansion_only_hold = bool(finance_gate_pass and not treasury_gate_pass and trading_finance_policy["hold_live_treasury"])
    latest["finance_gate_pass"] = finance_gate_pass
    latest["baseline_live_trading_pass"] = finance_gate_pass
    latest["treasury_gate_pass"] = treasury_gate_pass
    latest["capital_expansion_only_hold"] = capital_expansion_only_hold
    latest["finance_state"] = (
        result.get("finance_state")
        or live_hold.get("state")
        or ("queue_ready" if finance_gate_pass else "hold")
    )
    latest["capital_expansion_policy"] = trading_finance_policy
    latest["finance_gate"] = _finance_gate_snapshot(
        rollout_gates=rollout_gates,
        ...
        capital_expansion_only_hold=capital_expansion_only_hold,
        stage_cap=trading_finance_policy["stage_cap"],
        requested_mode=result.get("requested_mode") or result.get("mode"),
        reason=(
            live_hold.get("reason")
            or (trading_finance_policy["reason"] if capital_expansion_only_hold else None)
        ),
        remediation=(
            live_hold.get("remediation")
            or (trading_finance_policy["remediation"] if capital_expansion_only_hold else None)
        ),
        retry_at=live_hold.get("retry_at"),
        retry_in_minutes=live_hold.get("retry_in_minutes"),
        destination=live_hold.get("destination"),
        policy_checks=live_hold.get("policy_checks") or result.get("policy_checks"),
    )
    settings.latest_report_path.write_text(json.dumps(latest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    store.record_snapshot("execute", "finance_execute.v1", result)
    return result
