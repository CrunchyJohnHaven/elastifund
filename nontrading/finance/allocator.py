...
from nontrading.finance.policy import build_finance_totals, load_json
...
def _load_trading_signal(workspace_root: Path) -> tuple[float, float, list[str]]:
    ...
    baseline_pnl = float(maker.get("guardrail_recommendation", {}).get("baseline_live_filled_pnl_usd", 0.0) or 0.0)
    live_rows = float(maker.get("guardrail_recommendation", {}).get("baseline_live_filled_rows", 0.0) or 0.0)
...
def build_allocation_plan(store: FinanceStore, settings: FinanceSettings, audit_report: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    buckets = [
        ...
        AllocationBucket(
            ...
            recommended_amount_usd=(
                0.0
                if release_context["trading_blockers"] or release_context["executed_trading_allocation_usd"] > 0
                else capped_spend if trading_value > 0 and liquid_capacity > 0 else 0.0
            ),
            monthly_commitment_usd=0.0,
            rationale=(
                "Treat the executed $250 trading allocation as sufficient for the current proof window; hold incremental capital until service health, package load, executed notional, and conversion all recover."
    ...
    for bucket in ranked:
        if bucket.action_type and (bucket.recommended_amount_usd > 0 or bucket.bucket == "cut_or_cancel"):
