...
Usage:
    python -m simulator.run_sim run                          # Run with default config
    python -m simulator.run_sim run --config custom.yaml     # Run with custom config
    python -m simulator.run_sim run --mode maker             # Override execution mode
    python -m simulator.run_sim run --sizing kelly           # Override sizing method
    python -m simulator.run_sim audit                        # Run and produce audit summary
    python -m simulator.run_sim compare                      # Compare taker vs maker
"""
...
if __package__ in {None, ""}:  # pragma: no cover - script compatibility
    ROOT = Path(__file__).resolve().parents[1]
    if str(ROOT) not in sys.path:
        sys.path.insert(0, str(ROOT))
    from simulator.api import PaperTradeSimulator
    from simulator.metrics import SimulationReport
else:  # pragma: no cover - package import path
    from .api import PaperTradeSimulator
