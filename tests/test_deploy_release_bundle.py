...
from scripts.deploy_release_bundle import (
    ...
    build_runtime_state_alignment,
    build_remote_paper_commands,
    compare_env_keys,
    create_release_bundle,
    list_present_exact_deployable_files,
    load_release_plan,
    normalize_service_state,
    scan_profile_selector_definitions,
...
def test_list_present_exact_deployable_files_picks_up_ignored_env_overrides(tmp_path: Path) -> None:
    (tmp_path / "state").mkdir()
    (tmp_path / "state" / "btc5_capital_stage.env").write_text("BTC5_CAPITAL_STAGE=1\n")
    (tmp_path / "state" / "btc5_autoresearch.env").write_text("BTC5_MAX_ABS_DELTA=0.00015\n")

    assert list_present_exact_deployable_files(tmp_path) == (
...
def test_scan_profile_selector_definitions_detects_duplicate_runtime_profile_entries(tmp_path: Path) -> None:
    (tmp_path / ".env.example").write_text("JJ_RUNTIME_PROFILE=shadow_fast_flow\n")
    (tmp_path / "state").mkdir()
    (tmp_path / "state" / "btc5_capital_stage.env").write_text(
        "BTC5_CAPITAL_STAGE=1\nJJ_RUNTIME_PROFILE=blocked_safe\n"
    )

    report = scan_profile_selector_definitions(tmp_path)

    assert report["duplicate_detected"] is True
    assert report["total_occurrences"] == 2
    assert report["occurrences"] == [
        {"path": ".env.example", "count": 1},
        {"path": "state/btc5_capital_stage.env", "count": 1},
    ]
...
def test_build_release_contract_rejects_duplicate_runtime_profile_selectors(tmp_path: Path) -> None:
    _write_release_contract_files(tmp_path)
    (tmp_path / ".env.example").write_text("JJ_RUNTIME_PROFILE=shadow_fast_flow\n")
    (tmp_path / "state").mkdir(exist_ok=True)
    (tmp_path / "state" / "btc5_capital_stage.env").write_text("JJ_RUNTIME_PROFILE=blocked_safe\n")

    contract = build_release_contract(tmp_path)

    assert contract["valid"] is False
    assert contract["duplicate_profile_selector_detected"] is True
    assert contract["profile_selector_occurrences"] == [
        {"path": ".env.example", "count": 1},
        {"path": "state/btc5_capital_stage.env", "count": 1},
    ]
    assert any("JJ_RUNTIME_PROFILE is defined ambiguously" in issue for issue in contract["issues"])


def test_build_runtime_state_alignment_flags_profile_and_mode_mismatch() -> None:
    alignment = build_runtime_state_alignment(
        release_contract={
            "selected_profile": "paper_aggressive",
            "duplicate_profile_selector_detected": False,
        },
        remote_runtime_controls={
            "runtime_profile": "maker_velocity_all_in",
            "inferred_mode": "shadow",
        },
        runtime_profile_effective={
            "mode": {
    ...
    assert alignment["local_selected_profile"] == "paper_aggressive"
    assert alignment["remote_runtime_profile"] == "maker_velocity_all_in"
    assert alignment["profile_match"] is False
    assert alignment["mode_match"] is False
    assert any("remote runtime profile" in issue for issue in alignment["issues"])
    assert any("remote inferred mode" in issue for issue in alignment["issues"])
...
def test_build_release_manifest_keeps_btc5_capital_stage_env_when_present(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    (tmp_path / "state").mkdir()
    ...
    (tmp_path / ".env.example").write_text("JJ_RUNTIME_PROFILE=shadow_fast_flow\n")
    (tmp_path / "reports" / "root_test_status.json").write_text(
        json.dumps({"status": "passing", "summary": "22 passed", "checked_at": "2026-03-10T12:00:00Z"})
    )
    (tmp_path / "reports" / "remote_cycle_status.json").write_text(
    ...
    assert "state/btc5_capital_stage.env" in manifest["deploy_files"]
    assert manifest["exact_deploy_files_present"] == ["state/btc5_capital_stage.env"]
    assert manifest["checksums"]["state/btc5_capital_stage.env"] == hashlib.sha256(
        b"BTC5_CAPITAL_STAGE=1\n"
    ).hexdigest()
