...
def test_build_release_manifest_collects_checksums_and_runtime_truth(
    ...
    (tmp_path / "reports" / "remote_cycle_status.json").write_text(
        json.dumps({"launch": {"fast_flow_restart_ready": False}})
    )
    ...
    assert manifest["restart_recommended"] is True
    assert manifest["restart_source"] == "reports/edge_scan_20260309T000551Z.json"
    assert manifest["deploy_files"] == [".env.example", "bot/jj_live.py"]
    assert manifest["excluded_changed_files"] == ["docs/ignored.md"]
    assert manifest["checksums"]["bot/jj_live.py"] == hashlib.sha256(b"print('ok')\n").hexdigest()
    assert manifest["env_key_diff"]["added_in_cycle"] == ["NEW_KEY"]
...
def test_build_remote_paper_commands_uses_remote_dir() -> None:
    ...
    assert status_command.endswith("python3 bot/jj_live.py --status")
