...
def test_rank_dual_sided_spread_markets_prefers_locked_edge_and_low_toxicity() -> None:
    ranked = rank_dual_sided_spread_markets(
        [
            MarketSnapshot(
                market_id="m1",
                question="BTC 5m market one",
                yes_price=0.47,
                no_price=0.48,
    ...
    assert [row["market_id"] for row in ranked] == ["m1", "m2"]
    assert ranked[0]["combined_cost"] == 0.95
    assert ranked[0]["locked_edge"] == 0.05


def test_allocate_dual_sided_spread_notional_respects_caps() -> None:
    allocations = allocate_dual_sided_spread_notional(
        bankroll_usd=247.0,
        ranked_candidates=[
            {"market_id": "m1", "locked_edge": 0.03, "score": 0.05},
            {"market_id": "m2", "locked_edge": 0.025, "score": 0.04},
            {"market_id": "m3", "locked_edge": 0.02, "score": 0.03},
        ],
        reserve_pct=0.20,
    ...
    assert allocations == {"m1": 10.0, "m2": 10.0}


def test_build_dual_sided_spread_intents_builds_maker_only_pairs() -> None:
    intents = build_dual_sided_spread_intents(
        allocations_usd={"m1": 10.0},
        ranked_candidates=[
            {
                "market_id": "m1",
                "question": "BTC 5m dual-sided",
                "yes_price": 0.47,
                "no_price": 0.48,
                "combined_cost": 0.95,
                "locked_edge": 0.05,
    ...
    intent = intents[0]
    assert intent.maker_only is True
    assert intent.total_notional_usd == 10.0
    assert intent.per_side_notional_usd == 5.0
    assert intent.yes_side == "buy_yes"
    assert intent.no_side == "buy_no"
...
def test_validate_contract_payloads_and_kpis() -> None:
    ...
    ok_dual, reasons_dual = validate_contract_payload(
        "DualSidedSpreadIntent",
        {
            ...
            "per_side_notional_usd": 5.0,
            "yes_side": "buy_yes",
            "no_side": "buy_no",
            "maker_only": True,
            "timeout_seconds": 120,
            "max_inventory_skew_usd": 5.0,
            "wallet_confirmation_mode": "overlay_only",
        },
    )
    assert ok_dual is True
    assert reasons_dual == tuple()
