        ...
        *,
        ...
        legs = (
            A6LegSnapshot(
                ...
                condition_id="alice",
                token_id="alice-yes",
                no_token_id="alice-no",
                outcome_name="Alice",
                yes_bid=bids[0],
                yes_ask=asks[0],
                no_bid=1.0 - asks[0] if asks[0] is not None else None,
                no_ask=1.0 - bids[0] if bids[0] is not None else None,
                tick_size=0.01,
            ...
            A6LegSnapshot(
                ...
                yes_bid=bids[1],
                yes_ask=asks[1],
                no_bid=1.0 - asks[1] if asks[1] is not None else None,
                no_ask=1.0 - bids[1] if bids[1] is not None else None,
                tick_size=0.01,
                updated_ts=detected_at_ts,
                fresh=True,
                executable=executable,
            ...
            A6LegSnapshot(
                ...
                condition_id="carol",
                token_id="carol-yes",
                no_token_id="carol-no",
                outcome_name="Carol",
                yes_bid=bids[2],
                yes_ask=asks[2],
                no_bid=1.0 - asks[2] if asks[2] is not None else None,
                no_ask=1.0 - bids[2] if bids[2] is not None else None,
                tick_size=0.01,
                updated_ts=detected_at_ts,
                fresh=True,
                executable=executable,
        ...
        return A6MarketSnapshot(
            ...
            full_basket_guaranteed=executable,
            filtered_outcomes_present=False,
        )
