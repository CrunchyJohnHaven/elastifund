...
def test_build_openclaw_benchmark_packet_rolls_up_operational_metrics() -> None:
    ...
    assert packet.expected_arr_delta is None
    assert packet.improvement_velocity is None
    assert packet.candidate_confidence is None
    assert packet.metadata["routing"]["model_tier"] == "routine_ingestion"
    assert packet.metadata["routing"]["observed_model_tier"] == "routine_ingestion"
    assert packet.metadata["routing"]["estimated_compute_cost_usd"] == 0.06
    assert packet.metadata["routing"]["estimated_arr_uplift_per_model_minute"] is None
    assert packet.metadata["routing"]["estimated_confidence_gain"] == 0.0
...
def test_routing_plan_escalates_for_valuable_low_confidence_candidates() -> None:
    packet = build_openclaw_benchmark_packet(
        run_id="openclaw-esc-routing",
        diagnostics_events=[
            {"ts": 1_000, "type": "model.usage", "durationMs": 60_000, "costUsd": 0.5},
        ],
        outcome_comparisons=[
            {
                "case_id": "landing-page-cta",
                "elastifund_value": {"issues_found": 3},
                "openclaw_value": {"issues_found": 1},
                "winner": "comparison",
                "expected_arr_delta": 16.0,
    ...
    routing = packet.metadata["routing"]
    assert routing["model_tier"] == "structured_ranking"
    assert routing["estimated_arr_uplift_per_model_minute"] == 16.0
    assert routing["estimated_confidence_gain"] > 0
