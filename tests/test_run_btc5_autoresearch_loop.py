...
def test_write_loop_reports_accumulates_summary(tmp_path: Path) -> None:
    first_entry = {
        ...
        "package_class": "shadow_only",
        "package_candidate_class": "probe_only",
        "package_class_reason": "fresh_probe_blocks_promote",
        "package_class_reason_tags": ["validation_rows_flat"],
        "selected_package_class": "suppress",
        "selected_package_candidate_class": "suppress_cluster",
        "selected_package_class_reason": "fresh_probe_cluster_suppressed",
        "selected_package_class_reason_tags": ["recent_loss_cluster_flags_present"],
        ...
        "package_ranking": {
            "package_set_breakdown": {"live_candidate": 0, "shadow_only": 1, "hold_current": 0, "suppress": 0},
            "top_package_set": "shadow_only",
            "ranked_packages": [
                {
    ...
    assert latest_json["package_class"] == "shadow_only"
    assert latest_json["selected_package_class"] == "suppress"
    ...
    assert latest_json["package_ranking"]["package_set_breakdown"]["shadow_only"] == 1
    ...
    assert "Last package class" in latest_md
    assert "Last selected package class" in latest_md
    ...
    assert "Last top package set" in latest_md
...
def test_cadence_decision_accelerates_when_fresh_fills_hit_weak_regime() -> None:
    cadence = _cadence_decision(
        entry={
            "selected_package_class": "suppress",
            "current_probe": {
        ...
        previous_entry={
            "selected_package_class": "hold_current",
            "current_probe": {
                "live_filled_row_count": 10,
    ...
    assert cadence["mode"] == "accelerated"
    assert cadence["reason"] == "fresh_fills_arrived_in_weak_or_changed_regime"
    assert cadence["recommended_interval_seconds"] <= 90
    assert cadence["package_class_changed"] is True


def test_cadence_decision_slows_further_when_package_class_is_stable_and_flat() -> None:
    cadence = _cadence_decision(
        entry={
            "selected_package_class": "suppress",
            "current_probe": {
                "probe_freshness_hours": 1.25,
                "live_filled_rows_delta": 0,
                "validation_live_filled_rows_delta": 0,
            },
        },
    ...
    assert cadence["mode"] == "slowed"
    assert cadence["reason"] == "evidence_flat_and_package_class_stable"
    assert cadence["recommended_interval_seconds"] == 600
    assert cadence["recommended_interval_seconds"] > 300
