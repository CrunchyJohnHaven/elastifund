...
def test_build_dual_sided_shadow_plan_writes_valid_spread_intents(tmp_path: Path) -> None:
    repo = tmp_path
    markets = repo / "markets.json"
    markets.write_text(
        json.dumps(
    ...
    out_path = repo / "reports" / "dual_sided_shadow.json"
    result = _run(
    ...
    assert result.returncode == 0
    payload = json.loads(out_path.read_text())
    assert payload["strategy_family"] == "dual_sided_maker_spread_capture"
    assert payload["all_spread_intents_valid"] is True
    assert len(payload["ranked_candidates"]) == 1
    assert payload["ranked_candidates"][0]["market_id"] == "m1"
