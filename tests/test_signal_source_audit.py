...
def test_build_audit_payload_handles_empty_runtime(tmp_path, monkeypatch):
    state_path = tmp_path / "jj_state.json"
    _write_state(state_path, total_trades=0, cycles_completed=314, trade_log=[])
    monkeypatch.setattr(
        "scripts.run_signal_source_audit.DEFAULT_RUNTIME_TRUTH_PATH",
    ...
    assert payload["recommendations"]["wallet_flow"]["decision_mode"] == "data_collection_only"
    assert payload["recommendations"]["wallet_flow"]["decision_eligible"] is False
    assert payload["recommendations"]["microstructure_gate"]["recommendation"] == "keep_as_gate"
    assert payload["recommendations"]["microstructure_gate"]["decision_mode"] == "gate_only"
    assert "combined_sources_vs_single_source" in payload
    assert "ranking_snapshot" in payload
    assert payload["source_decision_readiness"]["wallet_flow"]["decision_mode"] == "data_collection_only"
    ...
    assert payload["capital_ranking_support"]["wallet_flow_archive_confirmation_ready"] is False
    assert payload["capital_ranking_support"]["lmsr_archive_confirmation_ready"] is False
    ...
    assert payload["capital_ranking_support"]["confirmation_coverage_label"] == "missing"
    assert payload["capital_ranking_support"]["confirmation_strength_label"] == "missing"
    assert payload["capital_ranking_support"]["confirmation_strength_score"] == 0.0
    ...
    assert "wallet_flow" in payload["capital_ranking_support"]["data_collection_only_sources"]
...
def test_build_audit_payload_compares_wallet_flow_against_llm(tmp_path):
    ...
    assert payload["recommendations"]["wallet_flow"]["decision_mode"] == "decision_eligible"
    assert payload["recommendations"]["wallet_flow"]["decision_eligible"] is True
    assert "wallet_flow" in payload["capital_ranking_support"]["decision_eligible_sources"]
...
def test_build_audit_payload_keeps_capital_allocation_ready_but_stage_upgrade_limited(tmp_path):
    ...
    assert capital_support["wallet_flow_archive_confirmation_ready"] is False
    assert capital_support["lmsr_archive_confirmation_ready"] is False
    ...
    assert capital_support["confirmation_coverage_label"] == "missing"
    assert capital_support["confirmation_strength_label"] == "missing"
    assert capital_support["confirmation_strength_score"] == 0.0
...
def test_build_audit_payload_emits_btc_fast_window_confirmation_metrics(tmp_path):
    ...
    assert confirmation["summary"]["confirmation_coverage_label"] == "strong"
    assert confirmation["summary"]["confirmation_freshness_label"] == "fresh"
    assert confirmation["summary"]["fresh_sources"] == ["wallet_flow"]
    assert confirmation["summary"]["stale_sources"] == []
    assert confirmation["summary"]["confirmation_next_required_artifact"] is None
    assert confirmation["summary"]["confirmation_strength_label"] == "strong"
    assert confirmation["summary"]["confirmation_strength_score"] > 0.8
    ...
    assert wallet_flow_summary["confirmation_coverage_label"] == "strong"
    assert wallet_flow_summary["confirmation_strength_label"] == "strong"
    assert wallet_flow_summary["source_freshness"] == "fresh"
    assert wallet_flow_summary["source_gap_vs_probe_hours"] < 0.01
    ...
    assert capital_support["wallet_flow_archive_confirmation_ready"] is True
    assert capital_support["lmsr_archive_confirmation_ready"] is False
    ...
    assert capital_support["confirmation_coverage_label"] == "strong"
    assert capital_support["confirmation_freshness_label"] == "fresh"
    assert capital_support["confirmation_fresh_sources"] == ["wallet_flow"]
    assert capital_support["confirmation_stale_sources"] == []
    assert capital_support["confirmation_next_required_artifact"] is None
    assert capital_support["confirmation_strength_label"] == "strong"
    assert capital_support["confirmation_strength_score"] > 0.8


def test_build_audit_payload_flags_stale_confirmation_sources(tmp_path):
    db_path = tmp_path / "jj_trades.db"
    state_path = tmp_path / "jj_state.json"
    probe_db_path = tmp_path / "btc_5min_maker.remote_probe.db"
    wallet_db_path = tmp_path / "wallet_scores.db"
    ...
    _write_btc5_probe_db(
        probe_db_path,
        [
            ("btc-updown-5m-30000", 30000, 30300, 30299, "DOWN", "live_filled", 0.48, 5.0, 1, "DOWN", 1, 5.0, 0.47, 0.49),
            ("btc-updown-5m-30300", 30300, 30600, 30599, "UP", "live_filled", 0.49, 5.0, 1, "DOWN", 0, -4.0, 0.48, 0.50),
            ("btc-updown-5m-30600", 30600, 30900, 30899, "DOWN", "live_filled", 0.47, 5.0, 1, "DOWN", 1, 3.0, 0.46, 0.48),
        ],
    )
    _write_wallet_flow_db(
        wallet_db_path,
        [
            ("wallet-a", 1, 12.0, 0.48, 100, "btc-updown-5m-100"),
            ("wallet-b", 1, 10.0, 0.47, 110, "btc-updown-5m-100"),
        ],
    )
    ...
    assert confirmation["status"] == "insufficient_data"
    assert confirmation["summary"]["confirmation_freshness_label"] == "missing"
    assert confirmation["summary"]["fresh_sources"] == []
    assert confirmation["summary"]["stale_sources"] == []
    assert confirmation["summary"]["confirmation_next_required_artifact"] == str(wallet_db_path)
    assert (
        confirmation["source_diagnostics"]["wallet_flow"]["source_gap_vs_probe_hours"]
        > 5.0
    )
    assert confirmation["source_diagnostics"]["wallet_flow"]["source_freshness"] == "unknown"
    assert capital_support["confirmation_freshness_label"] == "missing"
    assert capital_support["confirmation_stale_sources"] == []
    assert capital_support["confirmation_next_required_artifact"] == str(wallet_db_path)


def test_build_audit_payload_uses_report_and_bundle_fallbacks_when_local_dbs_are_missing(tmp_path, monkeypatch):
    state_path = tmp_path / "jj_state.json"
    _write_state(state_path, total_trades=0, cycles_completed=324, trade_log=[])

    remote_rows_path = tmp_path / "reports" / "tmp_remote_btc5_window_rows.json"
    remote_rows_path.parent.mkdir(parents=True, exist_ok=True)
    remote_rows_path.write_text(
        json.dumps(
            [
                {
    ...
    bundle_root = tmp_path / "reports" / "btc_intraday_llm_bundle_20260311T120000Z"
    _write_csv(
        bundle_root / "derived" / "wallet_consensus_btc5_windows.csv",
        [
            "event_slug",
            "consensus_direction",
            "total_volume",
        ...
        [
            ["btc-updown-5m-1", "DOWN", 12.0, 2, 2, 0.48, 389, 390, 12.0, 1.0],
            ["btc-updown-5m-2", "DOWN", 9.0, 2, 2, 0.46, 788, 790, 9.0, 1.0],
            ["btc-updown-5m-3", "DOWN", 7.0, 2, 2, 0.47, 1188, 1189, 7.0, 1.0],
        ],
    )
    _write_csv(
        bundle_root / "raw" / "wallet_wallet_trades.csv",
        [
            "id",
            "wallet",
            "condition_id",
        ...
        [
            [1, "wallet-a", "cond-1", "BTC 5m", "BUY", "Down", 1, 1, 12.0, 0.48, 390, 1, "btc-updown-5m-1", ""],
            [2, "wallet-b", "cond-2", "BTC 5m", "BUY", "Down", 1, 1, 9.0, 0.46, 790, 1, "btc-updown-5m-2", ""],
            [3, "wallet-c", "cond-3", "BTC 5m", "BUY", "Down", 1, 1, 7.0, 0.47, 1189, 1, "btc-updown-5m-3", ""],
        ],
    )
    _write_csv(
        bundle_root / "raw" / "edge_discovery_markets.csv",
        [
            "market_id",
            "condition_id",
            "slug",
        ...
        [
            [1, "cond-1", "btc-updown-5m-1", "5m", "BTC1", "chainlink", 100, 400, 100, 400, 0.48, "DOWN", 1, 1, "y1", "n1", 390, '{"volume": 1000}'],
            [2, "cond-2", "btc-updown-5m-2", "5m", "BTC2", "chainlink", 500, 800, 500, 800, 0.49, "DOWN", 1, 1, "y2", "n2", 790, '{"volume": 1000}'],
            [3, "cond-3", "btc-updown-5m-3", "5m", "BTC3", "chainlink", 900, 1200, 900, 1200, 0.47, "DOWN", 1, 1, "y3", "n3", 1190, '{"volume": 1000}'],
        ],
    )
    _write_csv(
        bundle_root / "raw" / "edge_discovery_trades.csv",
        ["trade_id", "condition_id", "timestamp_ts", "side", "outcome", "price", "size", "wallet", "tx_hash", "raw_json"],
        [
            ["t1", "cond-1", 380, "BUY", "Down", 0.48, 5, "wallet-a", "tx1", "{}"],
            ["t2", "cond-1", 381, "BUY", "Down", 0.48, 5, "wallet-b", "tx2", "{}"],
            ["t3", "cond-2", 780, "BUY", "Down", 0.46, 5, "wallet-a", "tx3", "{}"],
            ["t4", "cond-2", 781, "BUY", "Down", 0.46, 5, "wallet-b", "tx4", "{}"],
            ["t5", "cond-3", 1180, "BUY", "Down", 0.47, 5, "wallet-a", "tx5", "{}"],
            ["t6", "cond-3", 1181, "BUY", "Down", 0.47, 5, "wallet-b", "tx6", "{}"],
        ],
    )

    monkeypatch.setattr(
        "scripts.run_signal_source_audit.DEFAULT_REMOTE_BTC5_ROWS_PATH",
        remote_rows_path,
    )
    monkeypatch.setattr(
        "scripts.run_signal_source_audit.PROJECT_ROOT",
        tmp_path,
    )

    payload = build_audit_payload(
        db_path=tmp_path / "missing_jj_trades.db",
        state_path=state_path,
        minimum_signal_sample=50,
        btc5_probe_db_path=tmp_path / "data" / "btc_5min_maker.db",
        wallet_db_path=tmp_path / "data" / "wallet_scores.db",
        edge_db_path=tmp_path / "data" / "edge_discovery_locked.db",
    )

    confirmation = payload["btc_fast_window_confirmation"]
    wallet_summary = confirmation["by_source"]["wallet_flow"]
    lmsr_summary = confirmation["by_source"]["lmsr"]
    support = payload["capital_ranking_support"]

    assert confirmation["counts"]["resolved_window_rows"] == 3
    assert wallet_summary["source_window_rows"] == 3
    assert lmsr_summary["source_window_rows"] == 0
    assert "wallet_flow_db_missing_local_path_using_bundle" in " ".join(
        confirmation["source_diagnostics"]["wallet_flow"]["missing_requirements"]
    )
    assert "edge_discovery_db_missing_local_path_using_bundle" in " ".join(
        confirmation["source_diagnostics"]["lmsr"]["missing_requirements"]
    )
    assert "lmsr:source_window_rows 0 < required 3" in confirmation["missing_requirements"]
    assert "btc5_probe_db_missing:" not in " ".join(confirmation["missing_requirements"])
    assert "wallet_flow_db_missing:" in " ".join(confirmation["source_diagnostics"]["wallet_flow"]["missing_requirements"])
    assert support["confirmation_support_status"] == "ready"
    assert support["wallet_flow_archive_confirmation_ready"] is True
