...
pytest.skip(
    "Functions removed in refactor — tests need rewrite against new API",
...
def test_assemble_observed_rows_uses_cached_remote_rows_when_local_db_missing(tmp_path: Path) -> None:
    cache_path = tmp_path / "cached_rows.json"
    cache_path.write_text(
        json.dumps(
            [
                {
                    "id": 1,
                    "window_start_ts": 1773072000,
                    "slug": "btc-updown-5m-1773072000",
                    "direction": "DOWN",
                    "delta": -0.00010,
                    "order_price": 0.48,
                    "trade_size_usd": 5.0,
    ...
    rows, baseline = assemble_observed_rows(
        ...
        archive_glob="reports/does-not-exist/*.csv",
        refresh_remote=False,
        remote_cache_json=cache_path,
    )

    assert len(rows) == 1
    assert rows[0]["source"] == "remote_probe:cache"
    assert baseline["rows_by_source"] == {"remote_probe:cache": 1}
...
def test_run_monte_carlo_is_deterministic_for_same_seed(tmp_path: Path) -> None:
    ...
    assert first["non_positive_path_probability"] == first["non_positive_probability"]
    assert first["p95_drawdown_usd"] == first["p95_max_drawdown_usd"]
    assert first["active_trade_load"] == first["expected_active_trade_load"]
    assert first["tail_risk_summary"]["p95_drawdown_usd"] == first["p95_max_drawdown_usd"]
...
def test_build_summary_ranks_candidates(tmp_path: Path) -> None:
    ...
    assert capacity_stress["profiles"]["current_live_profile"]["trade_sizes_usd"] == [5.0, 10.0, 20.0, 50.0, 100.0, 300.0]
    ...
    assert summary["capital_ladder_summary"]["metric_name"] == "capital_ladder_summary"
    assert summary["capital_ladder_summary"]["recommended_reference"] == "best_candidate"
    assert summary["capital_ladder_summary"]["shadow_only"][1]["trade_size_usd"] == 300.0
    assert len(summary["shadow_trade_size_assessments"]) == 2
    assert summary["shadow_trade_size_assessments"][0]["trade_size_usd"] == 100.0
    assert "expected_order_failed_probability" in summary["shadow_trade_size_assessments"][0]
    assert "expected_post_only_retry_failure_rate" in summary["shadow_trade_size_assessments"][0]
    assert "evidence_verdict" in summary["shadow_trade_size_assessments"][0]
...
def test_build_capacity_stress_summary_emits_expected_size_sweeps(tmp_path: Path) -> None:
    ...
    assert stress["trade_sizes_usd"] == [5.0, 10.0, 20.0, 50.0, 100.0, 300.0]
    ...
    assert stress["stage_size_caps_usd"] == {
        "current": 5.0,
        "stage_1": 10.0,
        "stage_2": 20.0,
        "stage_3": 50.0,
    ...
    assert [item["shadow_label"] for item in stress["shadow_sweeps"]] == ["shadow_100", "shadow_300"]
    assert [item["trade_size_usd"] for item in stress["shadow_trade_size_assessments"]] == [100.0, 300.0]
    ...
    assert sweeps[5]["expected_fill_retention_ratio"] < sweeps[4]["expected_fill_retention_ratio"]
    assert sweeps[2]["fill_retention_drag"] == pytest.approx(0.5250, abs=1e-4)
    assert sweeps[2]["order_failed_rate_drag"] > 0.0
    assert sweeps[2]["post_only_retry_failure_rate_drag"] > 0.0
    ...
    assert "expected_loss_cluster_shock_hit_probability" in sweeps[2]
    ...
    assert "tail_risk_summary" in sweeps[2]
    assert "regime_size_sensitivity" in sweeps[2]
    assert sweeps[2]["session_size_sensitivity"][0]["session_name"] == "midday_et"
    assert sweeps[2]["tail_risk_summary"]["daily_loss_hit_probability"] == sweeps[2]["expected_daily_loss_hit_probability"]
    assert stress["stage_sweeps"][0]["trade_size_usd"] == 10.0
    stage_stress = stress["stage_stress_summary"]
    assert stage_stress["metric_name"] == "stage_stress_summary"
    assert stage_stress["stage_order"] == ["current", "stage_1", "stage_2", "stage_3"]
    assert stage_stress["stages_by_label"]["current"]["trade_size_usd"] == 5.0
    assert stage_stress["stages_by_label"]["stage_1"]["trade_size_usd"] == 10.0
    assert stage_stress["stages_by_label"]["stage_2"]["execution_drag_summary"]["fill_retention_drag"] == pytest.approx(
        0.5250,
        abs=1e-4,
    )
    assert stage_stress["stages_by_label"]["stage_3"]["tail_risk_summary"]["active_trade_load"] > 0.0
    capital_ladder = stress["capital_ladder"]
    assert capital_ladder["metric_name"] == "capital_ladder_summary"
    assert capital_ladder["live_now"]["safe_trade_size_usd"] >= 5.0
    assert capital_ladder["live_stage_decisions"][0]["stage_label"] == "current"
    assert [item["shadow_label"] for item in capital_ladder["shadow_only"]] == ["shadow_100", "shadow_300"]
    assert capital_ladder["shadow_only"][1]["trade_size_usd"] == 300.0
    assert capital_ladder["shadow_only"][0]["expected_order_failed_probability"] == pytest.approx(
        sweeps[4]["expected_order_failed_probability"],
        abs=1e-4,
    )
    assert capital_ladder["shadow_only"][0]["expected_post_only_retry_failure_rate"] == pytest.approx(
        sweeps[4]["expected_post_only_retry_failure_rate"],
        abs=1e-4,
    )
    assert capital_ladder["shadow_only"][0]["evidence_verdict"] in {
        "missing_evidence_only",
        "mixed_missing_and_true_negative",
        "true_negative_only",
        "tentatively_positive_but_underpowered",
        "sufficient_positive_evidence",
    }
    assert "missing_evidence_items" in capital_ladder["shadow_only"][0]
    assert "true_negative_items" in capital_ladder["shadow_only"][0]
    ...
    assert "tail_risk_summary" in stress["edge_tier_weighted_stage_sweeps"][0]
...
def test_build_capital_ladder_uses_stage_one_as_next_rung_when_current_is_blocked() -> None:
    summary = build_capital_ladder_summary(
        profile_name="runtime_recommended",
        historical={"replay_live_filled_rows": 12},
        reference_trade_size_usd=5.0,
        current_base_sweep={
            "trade_size_usd": 5.0,
            "expected_fill_retention_ratio": 0.98,
            "expected_p05_arr_pct": -10.0,
            "expected_loss_limit_hit_probability": 0.20,
            "expected_daily_loss_hit_probability": 0.20,
            "expected_non_positive_path_probability": 0.30,
            "expected_p95_max_drawdown_usd": 20.0,
        },
        stage_sweeps=[
            {
                "trade_size_usd": 10.0,
                "capital_stage": 1,
                "expected_fill_retention_ratio": 0.60,
                "expected_p05_arr_pct": -5.0,
                "expected_loss_limit_hit_probability": 0.30,
                "expected_daily_loss_hit_probability": 0.30,
                "expected_non_positive_path_probability": 0.35,
                "expected_p95_max_drawdown_usd": 35.0,
            },
            {
                "trade_size_usd": 20.0,
                "capital_stage": 2,
                "expected_fill_retention_ratio": 0.30,
                "expected_p05_arr_pct": -15.0,
                "expected_loss_limit_hit_probability": 0.50,
                "expected_daily_loss_hit_probability": 0.50,
                "expected_non_positive_path_probability": 0.45,
                "expected_p95_max_drawdown_usd": 60.0,
                "expected_post_only_retry_failure_rate": 0.30,
                "order_failed_rate_drag": 0.08,
            },
        ],
        shadow_sweeps=[],
    )

    assert summary["live_now"]["safe_trade_size_usd"] == 0.0
    assert summary["next_rung_floor_trade_size_usd"] == 5.0
    assert summary["next_notional_gate"]["stage_label"] == "stage_1"
    assert summary["next_notional_gate"]["trade_size_usd"] == 10.0
...
def test_run_monte_carlo_models_loss_cluster_shocks(tmp_path: Path) -> None:
    ...
    assert summary["tail_risk_summary"]["loss_cluster_shock_hit_probability"] == summary["loss_cluster_shock_hit_probability"]
