...
def test_bulk_flushes_documents_and_adds_timestamp():
    ...
    assert flushed_actions[0]["_source"]["event"]["dataset"] == "elastifund.trades"
    assert flushed_actions[0]["_source"]["service"]["name"] == "elastifund-bot"
    assert flushed_actions[0]["_source"]["ecs"]["version"] == "8.11.0"
    assert flushed_actions[1]["_source"]["event"]["dataset"] == "elastifund.signals"


def test_latency_documents_map_latency_ms_to_ecs_event_duration():
    flushed_actions: list[dict[str, object]] = []
    ...
    try:
        assert client.index_latency({"operation": "batch_submit", "latency_ms": 12.5, "ticker": "ETH"}) is True
        assert _wait_for(lambda: len(flushed_actions) == 1)
    finally:
        client.close()

    source = flushed_actions[0]["_source"]
    assert source["event"]["dataset"] == "elastifund.latency"
    assert source["event"]["duration"] == 12_500_000
    assert source["finance"]["ticker"] == "ETH"
...
def test_latency_index_template_uses_logsdb_settings():
    template_path = (
        Path(__file__).resolve().parents[1]
        / "infra"
        / "index_templates"
        / "elastifund-latency.json"
    )
    ...
    assert settings["index.mode"] == "logsdb"
    assert settings["index.codec"] == "best_compression"
    assert settings["index.sort.field"] == ["host.name", "@timestamp"]
    assert properties["event"]["properties"]["duration"]["type"] == "long"
    assert properties["host"]["properties"]["ip"]["synthetic_source_keep"] == "none"
