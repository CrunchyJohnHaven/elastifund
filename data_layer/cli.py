...
Usage:
    ...
    python -m data_layer flywheel-cycle --help
    python -m data_layer flywheel-bridge --help
"""
...
from flywheel.bridge import build_cycle_packet_from_bot_db, write_cycle_packet
...
from flywheel.runner import build_scorecard_from_db, load_cycle_packet, run_cycle
...
def cmd_flywheel_cycle(args):
    ...
    try:
        cycle_packet = load_cycle_packet(args.input)
        result = run_cycle(session, cycle_packet, artifact_root=args.artifact_dir)
    finally:
...
def cmd_flywheel_bridge(args):
    ...
    if args.output:
        path = write_cycle_packet(cycle_packet, args.output)
        print(path)
        return
    print(json.dumps(cycle_packet, indent=2, sort_keys=True))
...
def cmd_flywheel_naming_check(args):
    result = run_cycle_packet_naming_check(Path(args.repo_root))
    print(json.dumps(result, indent=2, sort_keys=True))
    if not result["ok"]:
        sys.exit(1)
...
def _register_base_commands(sub):
...
def _register_flywheel_commands(sub):
    p_cycle = sub.add_parser("flywheel-cycle", help="Run one sequential flywheel cycle from a JSON cycle packet")
    p_cycle.add_argument("--input", required=True, help="Path to cycle packet JSON")
    ...
    p_bridge = sub.add_parser("flywheel-bridge", help="Bridge an existing bot.db into a flywheel cycle packet")
    ...
    p_bridge.add_argument("--output", help="Optional output path for the generated cycle packet JSON")
    ...
    p_naming = sub.add_parser(
        "flywheel-naming-check",
        help="Guard cycle-packet modules against ambiguous 'payload' naming outside compatibility aliases",
    )
    p_naming.add_argument(
        "--repo-root",
        default=".",
        help="Repository root for static naming checks",
    )
...
def main(argv=None):
    ...
    _register_base_commands(sub)
    _register_flywheel_commands(sub)
