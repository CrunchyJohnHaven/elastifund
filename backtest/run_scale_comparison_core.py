...
from simulator.api import SimulatorEngine, load_simulator_config
...
from scripts.research_runtime import (
    load_json_dict as _shared_load_json_dict,
    normalize_mode,
)
...
from scripts.research_artifacts import (
    ...
    write_json_payload as _shared_write_json_payload,
)
...
DEFAULT_WALLET_RECONCILIATION_PATH = ROOT / "reports" / "wallet_reconciliation" / "latest.json"
DEFAULT_WALLET_EXPORT_DIR = Path.home() / "Downloads"
DEFAULT_WALLET_EXPORT_GLOB = "Polymarket-History-*.csv"
ANALYZE_DEFAULT_BANKROLL = [1000.0]
...
def _wallet_flow_archive_requires_rebuild(payload: dict[str, Any]) -> bool:
    """Treat cached feature-bundle schema failures as rebuildable/transient."""
    missing = [str(item) for item in payload.get("missing_requirements") or []]
    return any(item.startswith("feature_bundle_unavailable:") for item in missing)
    ...
    if archive_path and archive_path.exists():
        try:
            ...
            if _validate_wallet_flow_archive(payload):
                if _wallet_flow_archive_requires_rebuild(payload):
                    rebuilt = _build_wallet_flow_replay_archive(db_path)
...
def _load_json_dict(path: Path | None) -> dict[str, Any] | None:
    return _shared_load_json_dict(path)
...
def _build_audit_capital_support(
    ...
    *,
    ...
    wallet_flow_archive_confirmation_ready = bool(
        capital_support.get("wallet_flow_archive_confirmation_ready")
    )
    lmsr_archive_confirmation_ready = bool(capital_support.get("lmsr_archive_confirmation_ready"))
    ...
    confirmation_coverage_label = str(
        ...
        or _confirmation_coverage_label(
            status=confirmation_support_status,
            resolved_window_coverage=confirmation_resolved_window_coverage,
            executed_window_coverage=confirmation_executed_window_coverage,
        )
    )
    confirmation_strength_score = _safe_float(
        capital_support.get("confirmation_strength_score"),
        _confirmation_strength_score(
            status=confirmation_support_status,
            resolved_window_coverage=confirmation_resolved_window_coverage,
            executed_window_coverage=confirmation_executed_window_coverage,
            ...
            false_suppression_cost_usd=confirmation_false_suppression_cost_usd,
            false_confirmation_cost_usd=confirmation_false_confirmation_cost_usd,
        ),
    )
    confirmation_strength_label = str(
        capital_support.get("confirmation_strength_label")
        or _confirmation_strength_label(
            status=confirmation_support_status,
            coverage_label=confirmation_coverage_label,
            strength_score=confirmation_strength_score,
        )
    )
    ...
    return {
        ...
        "confirmation_strength_score": round(confirmation_strength_score, 6),
...
def _build_btc5_capital_confidence(
    *,
    live_scale_ready: bool,
    forecast_confidence_label: str,
    confirmation_support: dict[str, Any],
    audit_support: dict[str, Any],
) -> dict[str, Any]:
    confirmation_status = str(confirmation_support.get("status") or "unknown").lower()
    coverage_label = str(confirmation_support.get("coverage_label") or "missing").lower()
    strength_label = str(confirmation_support.get("strength_label") or "missing").lower()
    strength_score = round(_safe_float(confirmation_support.get("strength_score"), 0.0), 6)

    reasons: list[str] = []
    if not live_scale_ready:
        reasons.append("BTC5 does not currently clear the stage and capital gates for a live scale recommendation.")
        return {
            "label": "low",
    ...
    confidence_rank = _confidence_rank(forecast_confidence_label)
    if confirmation_status != "ready":
        confidence_rank = min(confidence_rank, 1)
        reasons.append("BTC fast-window confirmation is not fully ready, so live capital confidence stays capped below high.")
    else:
        if coverage_label in {"moderate", "weak"}:
            confidence_rank = min(confidence_rank, 1)
            reasons.append(
                f"BTC fast-window confirmation coverage is `{coverage_label}`, so live capital confidence is capped below high."
            )
        if strength_label == "weak":
            confidence_rank = min(confidence_rank, 1)
            reasons.append(
                "BTC fast-window confirmation shows weak realized support after contradiction and cost penalties."
            )
        elif strength_label == "strong":
            reasons.append("BTC fast-window confirmation provides strong realized support for the current BTC5 lane.")
        elif strength_label == "moderate":
            reasons.append("BTC fast-window confirmation is directionally supportive, but not yet strong enough to raise confidence further.")

    if not audit_support.get("fresh_for_capital_allocation"):
        confidence_rank = min(confidence_rank, 1 if live_scale_ready else 0)
        reasons.append("Signal-source audit freshness is outside the stage-upgrade window, so confidence stays capped.")

    return {
        "label": _confidence_label_from_rank(confidence_rank),
        "forecast_confidence_label": forecast_confidence_label,
        "confirmation_status": confirmation_status,
        "coverage_label": coverage_label,
        "strength_label": strength_label,
...
def _select_shadow_trade_size_assessment(
    *,
    ...
    assessment_sources = [
        capacity_profile.get("shadow_trade_size_assessments"),
        ((capacity_profile.get("capital_ladder") or {}).get("shadow_only")),
    ]
    for assessments in assessment_sources:
        if not isinstance(assessments, list):
            continue
        for assessment in assessments:
            if not isinstance(assessment, dict):
                continue
            if round(_safe_float(assessment.get("trade_size_usd"), 0.0), 4) == target_size:
                return assessment
    return None
...
def _build_shadow_size_recommendation(
    *,
    ...
    def _domain_entry(domain: str, checks: list[str], summary: str) -> dict[str, Any]:
        return {
            "domain": domain,
    ...
    if not capacity_profile:
        blocking_domains = [
            _domain_entry(
                "missing_truth",
                ["capacity_stress_summary_missing"],
        ...
        return {
            **base_payload,
            ...
            "decision_status": "missing_summary",
            "blocking_domains": blocking_domains,
            "blocking_domain_labels": [item["domain"] for item in blocking_domains],
        }
    ...
    if not isinstance(sweep, dict):
        ...
        return {
            **base_payload,
            ...
            "decision_status": "missing_sweep",
            "blocking_domains": blocking_domains,
            "blocking_domain_labels": [item["domain"] for item in blocking_domains],
        }

    assessment = _select_shadow_trade_size_assessment(
        trade_size_usd=trade_size_usd,
        capacity_profile=capacity_profile,
    )

    def _shadow_value(key: str) -> Any:
        if isinstance(assessment, dict) and key in assessment:
            return assessment.get(key)
        return sweep.get(key)

    fill_retention_ratio = _safe_float(_shadow_value("expected_fill_retention_ratio"), 0.0)
    order_failed_probability = _safe_float(_shadow_value("expected_order_failed_probability"), 0.0)
    retry_failure_rate = _safe_float(_shadow_value("expected_post_only_retry_failure_rate"), 0.0)
    p05_arr_pct = _safe_float(_shadow_value("expected_p05_arr_pct"), 0.0)
    loss_limit_hit_metric = _shadow_value("expected_loss_limit_hit_probability")
    loss_hit_probability = _safe_float(
        loss_limit_hit_metric
        if loss_limit_hit_metric is not None
        else _shadow_value("expected_loss_hit_probability"),
        0.0,
    )
    p95_drawdown_usd = _safe_float(_shadow_value("expected_p95_max_drawdown_usd"), 0.0)
    assessment_blocking_categories = (
        [str(item) for item in assessment.get("blocking_categories") or []]
        if isinstance(assessment, dict)
        else []
    )
    assessment_blocking_reasons = (
        [str(item) for item in assessment.get("blocking_reasons") or []]
        if isinstance(assessment, dict)
        else []
    )
    assessment_evidence_required = (
        [str(item) for item in assessment.get("evidence_required") or []]
        if isinstance(assessment, dict)
        else []
    )
    assessment_missing_evidence_items = (
        [str(item) for item in assessment.get("missing_evidence_items") or []]
        if isinstance(assessment, dict)
        else []
    )
    assessment_true_negative_items = (
        [str(item) for item in assessment.get("true_negative_items") or []]
        if isinstance(assessment, dict)
        else []
    )
    ...
    blocking_domains = [
        _domain_entry(
            ...
            (
                f"${trade_size_usd:.0f} stays shadow-only until higher-notional BTC5 evidence is replayed and "
                "confirmed in the live truth surface."
            ),
        )
    ]
    if fill_retention_ratio < 0.25:
        blocking_domains.append(
            _domain_entry(
                "liquidity",
                ["fill_retention_below_0.25"],
                (
                    f"Expected fill retention falls to {fill_retention_ratio:.2%} at ${trade_size_usd:.0f}, "
                    "so capacity still looks thin for live promotion."
                ),
            )
        )
    if order_failed_probability > 0.25 or retry_failure_rate > 0.20:
        checks: list[str] = []
        if order_failed_probability > 0.25:
            checks.append("order_failed_probability_above_0.25")
        if retry_failure_rate > 0.20:
            checks.append("post_only_retry_failure_rate_above_0.20")
        blocking_domains.append(
            _domain_entry(
                "execution_quality",
                checks,
                (
                    f"Execution drag remains too high at ${trade_size_usd:.0f}: "
                    f"order_failed_probability={order_failed_probability:.2%}, "
                    f"post_only_retry_failure_rate={retry_failure_rate:.2%}."
                ),
            )
        )
    if (
        p05_arr_pct <= 0.0
        or loss_hit_probability > 0.35
        or p95_drawdown_usd > max(25.0, trade_size_usd * 0.35)
    ):
        checks = []
        if p05_arr_pct <= 0.0:
            checks.append("p05_arr_non_positive")
        if loss_hit_probability > 0.35:
            checks.append("loss_limit_hit_probability_above_0.35")
        if p95_drawdown_usd > max(25.0, trade_size_usd * 0.35):
            checks.append("p95_drawdown_above_shadow_ceiling")
        blocking_domains.append(
            _domain_entry(
                "drawdown_tails",
                checks,
                (
                    f"Tail-risk gates still block ${trade_size_usd:.0f}: "
                    f"p05_arr_pct={p05_arr_pct:.2f}, "
                    f"loss_hit_probability={loss_hit_probability:.2%}, "
                    f"p95_drawdown_usd={p95_drawdown_usd:.2f}."
                ),
            )
        )
    ...
    blocking_checks = [
        check
        for item in blocking_domains
        for check in item.get("blocking_checks") or []
        if str(check).strip()
    ]
    blocking_domain_labels = [item["domain"] for item in blocking_domains]
    return {
        **base_payload,
        ...
        "decision_status": str(
            ...
            or ("shadow_ready" if len(blocking_domains) == 1 else "shadow_blocked")
        ),
        "deployment_class": str((assessment or {}).get("deployment_class") or "shadow_only"),
        "blocking_checks": _dedupe_preserve_order(blocking_checks),
        "reasons": [
            (
                f"Monte Carlo capacity stress models ${trade_size_usd:.0f} sizing with expected fill retention "
                f"{fill_retention_ratio:.2%}."
            ),
            "Keep this size in shadow/simulation only until live BTC5 stage evidence explicitly promotes it.",
        ],
        "blocking_domains": blocking_domains,
        "blocking_domain_labels": blocking_domain_labels,
        "blocking_categories": assessment_blocking_categories or blocking_domain_labels,
        "blocking_reasons": assessment_blocking_reasons or _dedupe_preserve_order(blocking_checks),
        "evidence_required": assessment_evidence_required,
        "evidence_verdict": (assessment or {}).get("evidence_verdict"),
        "missing_evidence_items": assessment_missing_evidence_items,
        "true_negative_items": assessment_true_negative_items,
        "execution_drag_summary": (assessment or {}).get("execution_drag_summary"),
        "tail_risk_summary": (assessment or {}).get("tail_risk_summary"),
        "expected_fill_probability": _shadow_value("expected_fill_probability"),
        "expected_fill_retention_ratio": _shadow_value("expected_fill_retention_ratio"),
        "expected_order_failed_probability": _shadow_value("expected_order_failed_probability"),
        "expected_post_only_retry_failure_rate": _shadow_value("expected_post_only_retry_failure_rate"),
        "expected_profit_probability": _shadow_value("expected_profit_probability"),
        "expected_loss_hit_probability": (
            _shadow_value("expected_loss_hit_probability")
            if _shadow_value("expected_loss_hit_probability") is not None
            else _shadow_value("expected_loss_limit_hit_probability")
        ),
        "expected_loss_limit_hit_probability": _shadow_value("expected_loss_limit_hit_probability"),
        "expected_non_positive_path_probability": _shadow_value("expected_non_positive_path_probability"),
        "expected_median_arr_pct": _shadow_value("expected_median_arr_pct"),
        "expected_p05_arr_pct": _shadow_value("expected_p05_arr_pct"),
        "expected_p95_max_drawdown_usd": _shadow_value("expected_p95_max_drawdown_usd"),
    }
...
def _load_wallet_reconciliation_summary(
    wallet_reconciliation_path: Path | None,
    *,
    now: datetime,
) -> dict[str, Any]:
    resolved_path = wallet_reconciliation_path
    empty = {
        "available": False,
        ...
        "btc_open_positions_total": None,
        "btc_open_positions_intentional": 0,
        "btc_open_positions_stale": 0,
        "orphan_remote_open_market_count": None,
        "phantom_csv_open_market_count": None,
        "remote_closed_missing_redeem_count": None,
    }
    ...
    if not payload:
        return empty
    summary = (
        payload.get("wallet_reconciliation_summary")
        if isinstance(payload.get("wallet_reconciliation_summary"), dict)
        else {}
    )
    btc_open_status = (
        payload.get("btc_open_status")
        if isinstance(payload.get("btc_open_status"), dict)
        else {}
    )
    cross_reference = (
        payload.get("cross_reference")
        if isinstance(payload.get("cross_reference"), dict)
        else {}
    )
    checked_at = summary.get("checked_at") or payload.get("generated_at")
    freshness_hours = _freshness_hours(checked_at, now=now)
    status_text = str(summary.get("status") or payload.get("status") or "unknown").strip().lower()
    return {
        "available": True,
        "path": str(resolved_path),
        ...
        "btc_open_positions_total": int(
            _safe_float(
                btc_open_status.get("btc_open_positions_total"),
                summary.get("open_positions_count"),
            )
            or 0
        ),
        "btc_open_positions_intentional": int(
            _safe_float(btc_open_status.get("btc_open_positions_intentional"), 0.0) or 0
        ),
        "btc_open_positions_stale": int(
            _safe_float(btc_open_status.get("btc_open_positions_stale"), 0.0) or 0
        ),
        "orphan_remote_open_market_count": int(
            _safe_float(cross_reference.get("orphan_remote_open_market_count"), 0.0) or 0
        ),
        "phantom_csv_open_market_count": int(
            _safe_float(cross_reference.get("phantom_csv_open_market_count"), 0.0) or 0
        ),
        "remote_closed_missing_redeem_count": int(
            _safe_float(cross_reference.get("remote_closed_missing_redeem_count"), 0.0) or 0
        ),
    }
...
def _build_stage_readiness(
    *,
    ...
    deploy_recommendation: str,
    confidence_label: str,
    best_candidate_class: str = "",
    best_package_arr_pct: float = 0.0,
    ...
    reconciliation = (
        ...
        if isinstance(wallet_reconciliation_summary, dict)
        else {}
    )
    reconciliation_available = bool(reconciliation.get("available"))
    reconciliation_status = str(reconciliation.get("status") or "").strip().lower()
    reconciliation_ready = reconciliation_status in {"reconciled", "ready_for_launch_gate"}
    reconciliation_fresh = bool(reconciliation.get("fresh_for_stage_upgrade")) if reconciliation_available else True
    btc_open_markets_total = int(
        _safe_float(reconciliation.get("btc_open_positions_total"), btc_open_markets)
    )
    btc_open_markets_intentional = int(
        _safe_float(reconciliation.get("btc_open_positions_intentional"), 0.0) or 0
    )
    btc_open_markets_stale = int(
        _safe_float(reconciliation.get("btc_open_positions_stale"), 0.0) or 0
    )
    ...
    wallet_stage_truth_ready = False
    forecast_promotes = (
        (deploy_recommendation == "promote" and confidence_label == "high")
        or (
            best_candidate_class == "promote"
            and confidence_label == "high"
            and best_package_arr_pct > 0
        )
    )
    if not wallet_available:
        ...
        if reconciliation_available:
            if not reconciliation_fresh:
                blocking_checks.append("wallet_reconciliation_stale")
            if btc_open_markets_stale > 0:
                blocking_checks.append("wallet_export_btc_open_markets_stale_not_zero")
            elif btc_open_markets_intentional > 0:
                reasons.append(
                    "Intentional BTC opens are still live and are tracked as non-blocking for stage reconciliation."
                )
            elif btc_open_markets_total != 0:
            ...
            wallet_stage_truth_ready = bool(
                wallet_fresh
                and btc_closed_markets > 0
                and reconciliation_fresh
                and reconciliation_ready
                and btc_open_markets_stale == 0
            )
        else:
            if btc_open_markets != 0:
                blocking_checks.append("wallet_export_btc_open_markets_not_zero")
            wallet_stage_truth_ready = bool(
                wallet_fresh
                and btc_closed_markets > 0
            ...
            if not wallet_stage_truth_ready:
                blocking_checks.append("stage_1_wallet_reconciliation_not_ready")

    if wallet_stage_truth_ready:
        reasons.append("Wallet reconciliation is ready for stage gates.")
    forecast_gate_pass = forecast_promotes
    ready_for_stage_1 = bool(
        wallet_stage_truth_ready
    ...
    return {
        ...
        "wallet_reconciliation_status": reconciliation_status or "unknown",
        "wallet_reconciliation_fresh": reconciliation_fresh,
        "wallet_reconciliation_ready": reconciliation_ready if reconciliation_available else None,
        "wallet_reconciliation_source": reconciliation.get("path"),
        "btc_open_markets_total": btc_open_markets_total,
        "btc_open_markets_intentional": btc_open_markets_intentional,
        "btc_open_markets_stale": btc_open_markets_stale,
...
def _build_btc5_venue_entry(
    *,
    ...
    wallet_reconciliation_summary = _load_wallet_reconciliation_summary(
    ...
    best_candidate_class = str(
        (forecast_payload.get("best_candidate") or {}).get("candidate_class", "")
    ).lower()
    best_package_arr_pct = _safe_float(
        (forecast_payload.get("arr_tracking") or {}).get("best_median_arr_pct")
        or (forecast_payload.get("best_candidate", {}).get("continuation") or {}).get("median_arr_pct"),
        0.0,
    )
    ...
    stage_readiness = _build_stage_readiness(
        ...
        deploy_recommendation=deploy_recommendation,
        confidence_label=confidence_label,
        best_candidate_class=best_candidate_class,
        best_package_arr_pct=best_package_arr_pct,
    ...
    confirmation_support = {
        ...
        "coverage_label": audit_support.get("confirmation_coverage_label"),
        "strength_label": audit_support.get("confirmation_strength_label"),
    ...
    if confirmation_support.get("status") == "ready":
        ...
        ranking_score += 10.0 * (
            _safe_float(confirmation_support.get("strength_score"), 0.0) - 0.5
        )
    elif audit_support.get("available"):
        ranking_score -= 12.0

    coverage_label = str(confirmation_support.get("coverage_label") or "missing").lower()
    if coverage_label == "moderate":
        ranking_score -= 6.0
    elif coverage_label == "weak":
        ranking_score -= 15.0
    ...
    capital_confidence = _build_btc5_capital_confidence(
    ...
    return {
        "venue": "polymarket",
        "lane": "btc5",
        "confidence_label": capital_confidence.get("label"),
        ...
        "source_artifacts": [
            ...
            for path in (
                ...
                wallet_reconciliation_summary.get("path"),
...
def _build_capital_allocation_recommendation(
    ...
    *,
    ...
    elif next_100_shadow.get("status") == "shadow_only" or next_300_shadow.get("status") == "shadow_only":
    ...
    *,
    ...
    venue_scoreboard = [
        _build_btc5_venue_entry(
            ...
            wallet_reconciliation_path=wallet_reconciliation_path,
...
def write_wallet_flow_archive(archive: dict[str, Any], output_path: Path) -> None:
    ...
    _shared_write_json_payload(output_path, archive)


def render_markdown(report: dict[str, Any]) -> str:
    """Render the scale-comparison payload as Markdown."""

    def _shadow_ladder_metrics_line(item: dict[str, Any]) -> str:
        verdict = item.get("evidence_verdict") or "unknown"
        return (
            "  - Ladder metrics: "
            + f"fill retention {_safe_float(item.get('expected_fill_retention_ratio'), 0.0):.2%}, "
            + f"order-failed {_safe_float(item.get('expected_order_failed_probability'), 0.0):.2%}, "
            + f"retry fail {_safe_float(item.get('expected_post_only_retry_failure_rate'), 0.0):.2%}, "
            + f"P05 ARR {_safe_float(item.get('expected_p05_arr_pct'), 0.0):.2f}%, "
            + f"P95 drawdown ${_safe_float(item.get('expected_p95_max_drawdown_usd'), 0.0):.2f}, "
            + f"verdict `{verdict}`."
        )
    ...
    if capital_allocation:
        ...
        for reason in next_100_shadow.get("reasons") or []:
            lines.append(f"  - {reason}")
        if next_100_shadow.get("status") == "shadow_only":
            lines.append(_shadow_ladder_metrics_line(next_100_shadow))
            if next_100_shadow.get("blocking_domain_labels"):
                lines.append(
                    "  - Blocking domains: "
                    + ", ".join(str(label) for label in next_100_shadow.get("blocking_domain_labels") or [])
                    + "."
                )
        lines.append(
        ...
        for reason in next_300_shadow.get("reasons") or []:
            lines.append(f"  - {reason}")
        if next_300_shadow.get("status") == "shadow_only":
            lines.append(_shadow_ladder_metrics_line(next_300_shadow))
            if next_300_shadow.get("blocking_domain_labels"):
                lines.append(
                    "  - Blocking domains: "
                    + ", ".join(str(label) for label in next_300_shadow.get("blocking_domain_labels") or [])
                    + "."
                )
        lines.append("")
...
def run_scale_comparison(
    ...
    wallet_reconciliation_path: Path | None = None,
...
def parse_args() -> argparse.Namespace:
    ...
    add_mode_argument(
        parser,
        help_text=(
            "Execution mode: full performs normal archive handling; analyze/quick skip archive rebuild "
            "and default to one bankroll."
        ),
    )
    parser.add_argument(
        "--bankrolls",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=DEFAULT_WALLET_RECONCILIATION_PATH,
        help=(
            "Optional reports/wallet_reconciliation/latest.json path used to classify BTC opens as "
            "intentional vs stale for stage-gate evaluation."
        ),
    )
    parser.add_argument(
        "--btc5-current-probe",
...
def _effective_bankrolls_for_mode(mode: str, bankrolls: list[float]) -> list[float]:
    if mode == "analyze" and bankrolls == DEFAULT_BANKROLLS:
        return ANALYZE_DEFAULT_BANKROLL
    return bankrolls


def _prepare_wallet_flow_archive_for_mode(mode: str, archive_path: Path) -> None:
    if mode == "full":
        ...
        write_wallet_flow_archive(archive, archive_path)
        return
    if archive_path.exists():
        return
    archive_path.parent.mkdir(parents=True, exist_ok=True)
    write_wallet_flow_archive(
        {
            "schema_version": WALLET_FLOW_ARCHIVE_SCHEMA,
            "generated_at": _utc_now().isoformat(),
            "resolved_signal_rows": [],
            "sample_size_summary": _sample_size_summary([]),
            "timebound_evidence_window": _empty_timebound_evidence_window("resolved_signals"),
            "gaps": ["analyze_mode_archive_missing"],
            "notes": ["Analyze mode skipped DB rebuild because archive was missing."],
        },
        archive_path,
    )
...
def main() -> None:
    ...
    effective_bankrolls = _effective_bankrolls_for_mode(mode, args.bankrolls)

    archive_path = args.wallet_flow_archive
    _prepare_wallet_flow_archive_for_mode(mode, archive_path)
    report = run_scale_comparison(
        bankrolls=effective_bankrolls,
        json_output_path=args.json_output,
        markdown_output_path=args.markdown_output,
        wallet_flow_archive_path=archive_path,
        ...
        wallet_reconciliation_path=args.wallet_reconciliation,
