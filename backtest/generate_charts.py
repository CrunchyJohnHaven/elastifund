...
def chart_calibration(backtest):
    ...
    ax.set_title(f"AI Model Calibration\n{backtest['markets_analyzed']} Resolved Markets", fontsize=14, fontweight="bold")
...
def chart_equity_curve(markets, cache):
    ...
    n_trades = trade_num
    ax.set_title(f"Simulated Equity Curve\n{n_trades} Trades Across {len(markets)} Resolved Markets", fontsize=14, fontweight="bold")
    ax.legend(fontsize=11)
...
def chart_win_rate_by_direction(backtest):
    ...
    ax.set_title(f"Win Rate by Trade Direction\n{backtest['markets_analyzed']} Resolved Markets", fontsize=14, fontweight="bold")
    ax.set_ylim(0, 95)
